...
#         """Récupère un véhicule LOMETA (objet, pas dict)"""
#         try:
#             return self.main_controller.vehicles.get_vehicles_by_id(vehicle_id)
#         except Exception as e:
#             logger.error(f"Erreur get_vehicle_by_id: {e}")
#             return None

#     def get_contract_by_vehicle(self, vehicle_id: int):
#         """Récupère le contrat d'un véhicule (objet, pas dict)"""
#         try:
#             return self.main_controller.contracts.get_contract_by_vehicle(vehicle_id)
#         except Exception as e:
#             logger.error(f"Erreur get_contract_by_vehicle: {e}")
#             return None

#     def get_contact_by_id(self, contact_id: int):
#         """Récupère un contact LOMETA (objet, pas dict)"""
#         try:
#             return self.main_controller.contacts.get_contact_by_id(contact_id)
#         except Exception as e:
...
#         """
#         try:
#             # 1. Récupérer les données (objets, pas dict)
#             vehicle = self.get_vehicle_by_id(vehicle_id)
#             if not vehicle:
#                 logger.error(f"Véhicule {vehicle_id} non trouvé")
#                 return None
            
#             contract = self.get_contract_by_vehicle(vehicle_id)
#             if not contract:
...
#             contact = self.get_contact_by_id(owner_id)
#             if not contact:
...
#         """Construit un élément de production complet (travaille avec des objets)"""
...
#         # Dates (travaille avec des objets, pas des dict)
#         starts_at = getattr(contract, 'date_debut', None) or date.today()
...
#             "rc": int(getattr(contract, 'prime_totale_ttc', 0) or 0),
...
#             "customer_phone": getattr(contact, 'telephone', '') or "690000000",
...
#             "insured_phone": getattr(contact, 'telephone', '') or "690000000",
...
#             "insured_birthdate": getattr(contact, 'date_naissance', None).isoformat() if getattr(contact, 'date_naissance', None) else None,
...
#             "driver_birthdate": (getattr(contact, 'date_naissance', None) or date(1990, 1, 1)).isoformat(),
...
#         contract.prime_totale_ttc = 50000
#         return contract

#     def _get_profession_code(self, contact) -> str:
#         """Retourne le code profession ASAC"""
#         profession_map = {
#             "Commerçant": "ST01",
