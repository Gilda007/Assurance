...
from PySide6.QtCore import Qt, QDateTime, Signal, QSize, QDate, QTime
...
class SinistreFormView(QDialog):
    ...
    #     # Type
    #     index = self.type_combo.findData(sinistre.type if sinistre.type else "")
    #     if index >= 0:
    #         self.type_combo.setCurrentIndex(index)
        
    #     # Date
    #     if sinistre.date_survenue:
    #         self.date_survenue.setDateTime(QDateTime.toPython(sinistre.date_survenue))
    ...
    def load_data(self):
        ...
        # ✅ CORRECTION : Convertir datetime en QDateTime
        if sinistre.date_survenue:
            qdatetime = QDateTime()
            qdatetime.setDate(
                QDate(
                    sinistre.date_survenue.year,
                    sinistre.date_survenue.month,
                    sinistre.date_survenue.day
                )
            )
            qdatetime.setTime(
                QTime(
                    sinistre.date_survenue.hour,
                    sinistre.date_survenue.minute,
                    sinistre.date_survenue.second
                )
            )
            self.date_survenue.setDateTime(qdatetime)
        # Lieu
        self.lieu_edit.setText(sinistre.lieu or "")
        self.ville_edit.setText(sinistre.ville or "")
        self.pays_edit.setText(sinistre.pays or "Cameroun")
    ...
    def save_sinistre(self):
        ...
        try:
            if self.is_edit:
                success = self.controller.sinistre.update(self.sinistre.id, data)
                message = "Sinistre mis à jour avec succès"
                sinistre = self.sinistre
            else:
                sinistre = self.controller.sinistre.create(data)
