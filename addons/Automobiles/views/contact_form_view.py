...
class ContactForm(QDialog):
    ...
    def get_data(self):
        """
        Récupère les champs nettoyés. 
        Note : Les clés doivent correspondre aux noms des variables dans Contact (contact_models.py)
        """
        return {
            # I. Administration & Statut
            "type_client": self.type_client.currentText(),
            "nature": self.nature.currentText(),
            # Utilisez la clé sans accent ici pour correspondre à la variable Python du modèle
            "charge_clientele": self.charge_client.text().strip(), 
            "redacteur_production": self.redacteur.text().strip(),
            "vip_status": "Non", # Ajoutez une valeur par défaut ou liez à un widget
            
            # II. État Civil & Identité
            "civilite": self.civilite.currentText(),
            "nom": self.nom.text().strip().upper(),
            "prenom": self.prenom.text().strip().title(),
            ...
            # Image passée à part pour traitement dans le contrôleur
