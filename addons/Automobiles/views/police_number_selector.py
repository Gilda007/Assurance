...
#         self.load_compagnie_info()
        
#         self.setup_ui()
#         self.load_numbers()

#     def load_compagnie_info(self):
#         """Charge les informations de la compagnie"""
#         try:
#             from addons.Automobiles.models import Compagnie
#             self.compagnie = self.controller.session.query(Compagnie).filter(
#                 Compagnie.id == self.compagnie_id
#             ).first()
...
#         company_name = self.compagnie.nom if self.compagnie else "Compagnie"
#         title = QLabel(f"Attribution du numéro de police - {company_name}")
#         title.setStyleSheet("""
#             font-size: 18px;
#             font-weight: 700;
#             color: white;
...
#         range_layout.setContentsMargins(16, 12, 16, 12)
#         range_layout.setSpacing(16)
        
#         if self.compagnie:
#             range_text = f"📋 Plage: {self.compagnie.num_debut} → {self.compagnie.num_fin}"
#         else:
#             range_text = "📋 Plage: non définie"
        
#         range_label = QLabel(range_text)
#         range_label.setStyleSheet("font-size: 13px; font-weight: 600; color: #1e293b;")
#         range_layout.addWidget(range_label)
#         range_layout.addStretch()
        
#         # Code de la compagnie
#         if self.compagnie:
#             code_label = QLabel(f"Code: {self.compagnie.code}")
...
#                 border-radius: 20px;
#             """)
#             range_layout.addWidget(code_label)
        
#         body_layout.addWidget(range_frame)
...
#         format_label = QLabel("Format: CODE-ANNÉE-NUMÉRO")
#         format_label.setStyleSheet("font-size: 11px; color: #94a3b8;")
#         self.numbers_layout.addWidget(format_label)
...
class PoliceNumberSelector(QDialog):
    ...
    number_selected = Signal(int, str)  # numero_police_attribue, numero_police_complet
    ...
    def setup_ui(self):
        ...
        # Titre avec nom de la compagnie
        company_name = self.compagnie.nom if self.compagnie else "Compagnie"
        title = QLabel(f"Attribution du numéro de police - {company_name}")
        title.setStyleSheet("""
            font-size: 18px;
        ...
        self.progress_bar.setStyleSheet("""
            ...
            QProgressBar::chunk {
                background: qlineargradient(x1:0, y1:0, x2:1, y2:0,
                    stop:0 #10b981, stop:0.5 #f59e0b, stop:1 #ef4444);
                border-radius: 6px;
            }
        """)
        body_layout.addWidget(self.progress_bar)
        ...
        selection_layout.addWidget(self.number_combo)
        selection_layout.addStretch()
        
        # Label pour le format
        format_label = QLabel("Format: CODE-YYYYMM-NUMÉRO")
        format_label.setStyleSheet("font-size: 11px; color: #94a3b8;")
        selection_layout.addWidget(format_label)
        
        body_layout.addWidget(selection_container)
        ...
        try:
            ...
            if not self.compagnie:
                self.warning_label.setText(
                    ...
                    "Veuillez sélectionner une compagnie valide."
                )
                self.warning_label.setVisible(True)
                self.confirm_btn.setEnabled(False)
                return
            ...
            if not self.compagnie.num_debut or not self.compagnie.num_fin:
                self.warning_label.setText(
                    "⚠️ Plage de numéros non définie pour cette compagnie.\n\n"
                    "Veuillez configurer les champs 'Numéro début' et 'Numéro fin' "
                    "dans les paramètres de la compagnie."
                )
                self.warning_label.setVisible(True)
                self.confirm_btn.setEnabled(False)
            ...
            available, total, used = self.controller.vehicles.get_available_numbers(self.compagnie_id)
            ...
            if available:
                for num in available:
                    formatted = self.controller.vehicles.generate_police_number(self.compagnie_id, num)
                    self.number_combo.addItem(f"{num:04d} → {formatted}", num)
                ...
                self.warning_label.setText(
                    "⚠️ ATTENTION : La plage de numéros de cette compagnie est épuisée !\n\n"
                    f"Plage configurée: {self.compagnie.num_debut} → {self.compagnie.num_fin}\n"
                    f"Total: {total} numéros, Utilisés: {used}\n\n"
                    "Veuillez contacter l'administrateur pour étendre la plage de numéros."
                )
                self.warning_label.setVisible(True)
            ...
            print(f"Erreur load_numbers: {e}")
            self.warning_label.setText(f"❌ Erreur de chargement: {str(e)}")
            self.warning_label.setVisible(True)
    ...
    def on_number_selected(self, index):
        ...
        if index >= 0 and self.number_combo.itemData(index):
            num = self.number_combo.itemData(index)
            police_number = self.controller.vehicles.generate_police_number(self.compagnie_id, num)
            self.preview_number.setText(police_number)
