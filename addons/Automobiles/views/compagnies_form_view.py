...
class FormulaireCreationCompagnie(QDialog):
    def __init__(self, parent=None, controller=None, current_user=None, data=None):
        ...
        if data:
            self.set_data(data)
            self.setWindowTitle("Modifier la Compagnie")
    ...
    def get_data(self):
        """Retourne les données nettoyées pour le contrôleur"""
        return {
            ...
            "num_debut": self.input_num_debut.text().strip(),
            "num_fin": self.input_num_fin.text().strip()
        }
    ...
    def set_data(self, data):
        """Remplit les champs et adapte l'UI pour la modification."""
        mapping = {
            'nom': self.input_nom,
            'code': self.input_code,
            'email': self.input_email,
            'telephone': self.input_telephone,
            'adresse': self.input_adresse,
            'num_debut': self.input_num_debut,
            'num_fin': self.input_num_fin
        }

        for key, field in mapping.items():
            if key in data:
                field.setText(str(data[key]) if data[key] is not None else "")

        # Verrouillage du code en mode édition
        self.input_code.setReadOnly(True)
        self.input_code.setStyleSheet(
            self.input_code.styleSheet() + 
            "background-color: #f8fafc; color: #64748b; border: 1px dashed #cbd5e1;"
        )

    def accept(self):
        """Vérifie les données avant de fermer le dialogue."""
        data = self.get_data()
        
        # --- VALIDATION DES CHAMPS OBLIGATOIRES ---
        errors = []
        if not data.get("code"):
            errors.append("- Le Code de la compagnie est requis.")
        if not data.get("nom"):
            errors.append("- Le Nom de la compagnie est requis.")
            
        if errors:
            # On affiche une petite alerte (vous pouvez utiliser votre AlertManager)
            from PySide6.QtWidgets import QMessageBox
            QMessageBox.warning(self, "Champs manquants", 
                                "Veuillez corriger les erreurs suivantes :\n" + "\n".join(errors))
            return # On annule la fermeture du dialogue

        # --- FEEDBACK VISUEL SI VALIDE ---
        from PySide6.QtGui import QCursor
        self.setCursor(QCursor(Qt.WaitCursor))
        self.btn_save.setEnabled(False)
        self.btn_save.setText("TRAITEMENT EN COURS...")
        
        # Tout est bon, on ferme avec le succès
        super().accept()

    def set_read_only(self, is_read_only=True):
        """Désactive tous les champs pour le mode consultation."""
        # Liste des suffixes utilisés dans _create_field
        attrs = ['code', 'nom', 'email', 'telephone', 'adresse', 'num_debut', 'num_fin']
        
        for attr in attrs:
            field = getattr(self, f"input_{attr}", None)
            if field:
                field.setReadOnly(is_read_only)
                if is_read_only:
                    # Style visuel "Lecture seule" : pas de bordure, fond gris très léger
                    field.setStyleSheet("background-color: #f8fafc; border: 1px solid #e2e8f0; color: #475569;")

        if is_read_only:
            self.btn_save.hide() # On cache le bouton enregistrer
            # On change le titre si vous avez un self.title_label
            if hasattr(self, 'title_label'):
                self.title_label.setText("DÉTAILS DE LA COMPAGNIE")