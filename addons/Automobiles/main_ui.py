...
# addons/Automobiles/main_ui.py - Version simplifiée de LoadingOverlay

# class LoadingOverlay(QFrame):
#     """Overlay de chargement SIMPLE et EFFICACE - sans animations complexes"""
...
#         # Spinner
#         self.spinner = QLabel("⏳")
#         self.spinner.setStyleSheet("font-size: 48px; background: transparent;")
#         self.spinner.setAlignment(Qt.AlignCenter)
#         card_layout.addWidget(self.spinner)
        
#         # Titre
#         self.title_label = QLabel("Chargement")
#         self.title_label.setStyleSheet("font-size: 20px; font-weight: 700; color: #1e293b;")
#         self.title_label.setAlignment(Qt.AlignCenter)
#         card_layout.addWidget(self.title_label)
        
#         # Message
#         self.message_label = QLabel("Veuillez patienter...")
#         self.message_label.setStyleSheet("font-size: 14px; color: #64748b;")
...
#         self.percentage_label = QLabel("0%")
#         self.percentage_label.setStyleSheet("font-size: 12px; color: #3b82f6; font-weight: 600;")
...
#     def show_loading(self, message="Chargement...", title=None):
#         """Affiche l'overlay"""
#         self.message_label.setText(message)
#         if title:
#             self.title_label.setText(title)
...
#         # Animation du spinner (rotation de l'emoji)
...
#             parent_rect = self.rect()
#             self.card.move(
#                 (parent_rect.width() - self.card.width()) // 2,
#                 (parent_rect.height() - self.card.height()) // 2 - 50
#             )
...
#         """Rotation du spinner"""
#         spinners = ["⏳", "⌛", "⏳", "⌛"]
#         self._spinner_step = (self._spinner_step + 1) % len(spinners)
#         self.spinner.setText(spinners[self._spinner_step])
...
#         self.percentage_label.setText(f"{value}%")
#         if step:
#             self.message_label.setText(step)
...
#             )
#         super().resizeEvent(event)
# addons/Automobiles/main_ui.py - Version ultra-simple

from Custom_Widgets.QCustomLoadingIndicators import QCustom3CirclesLoader

class LoadingOverlay(QFrame):
    """Overlay de chargement ultra-simple"""
    ...
    def __init__(self, parent=None):
        ...
        # ✅ Layout simple comme dans test.py
        layout = QVBoxLayout(self)
        layout.setAlignment(Qt.AlignCenter)
        
        # ✅ Loader comme dans test.py
        self.loader = QCustom3CirclesLoader(color=QColor("#3b82f6"))
...
#     #     except Exception as e:
#     #         self.error.emit(str(e))
# addons/Automobiles/main_ui.py - Version simplifiée de ModuleInitThread

class ModuleInitThread(QThread):
    """Thread simple pour charger les données en arrière-plan"""
    ...
    def run(self):
        try:
            ...
            self.progress.emit(10, "🔐 Vérification de la session...")
            
            # Compter les véhicules
            self.progress.emit(30, "🚗 Chargement des véhicules...")
            try:
                from addons.Automobiles.models import Vehicle
                vehicle_count = self.db_session.query(Vehicle).filter(
                    Vehicle.owner_id == user_id,
                ...
                self.progress.emit(50, f"🚗 {vehicle_count} véhicules")
            except Exception as e:
                self.progress.emit(50, f"🚗 {str(e)[:30]}")
            
            # Compter les contrats
            self.progress.emit(60, "📄 Chargement des contrats...")
            try:
                from addons.Automobiles.models import Contrat
                contrat_count = self.db_session.query(Contrat).filter(
                    Contrat.owner_id == user_id
                ).count()
                self.progress.emit(75, f"📄 {contrat_count} contrats")
            except Exception as e:
                self.progress.emit(75, f"📄 {str(e)[:30]}")
            
            # Créer le contrôleur (DANS LE THREAD)
            # ⚠️ ATTENTION: On crée le contrôleur ici, mais il doit être utilisé dans le thread principal
            self.progress.emit(85, "⚙️ Initialisation du contrôleur...")
            controller = AutomobileMainController(
                session=self.db_session,
                current_user_id=user_id
            )
            
            self.progress.emit(95, "🎨 Préparation de l'interface...")
            self.progress.emit(100, "✅ Prêt !")
            
            # Émettre le contrôleur
            self.finished.emit(controller)
...
class AutomobileModule(BaseModule):
    ...
    def __init__(self, main_window):
        ...
        self._loading_overlay = None
        self._init_thread = None
        # ✅ Ajouter un flag pour éviter les chargements multiples
    ...
    def activate_module(self):
        ...
        if self._is_loaded and self._view:
            self._show_existing_view()
            return
        ...
        loader = self._ensure_loading_overlay()
        if loader:
            # ✅ Supprimer simulate=True
            loader.show_loading("Chargement du module Automobile...", "Initialisation")
    ...
    def _on_module_initialized(self, controller, current_user, content_stack, loader):
        ...
        # ✅ Attendre la fin du thread avant de le supprimer
        if self._init_thread:
            if self._init_thread.isRunning():
                self._init_thread.wait(500)
            self._init_thread.deleteLater()
            self._init_thread = None
            
    def _show_existing_view(self):
        """Affiche la vue déjà chargée"""
        content_stack = self._get_content_area()
        if content_stack and self._view:
            if content_stack.indexOf(self._view) == -1:
                content_stack.addWidget(self._view)
            content_stack.setCurrentWidget(self._view)
