...
class UserModule(BaseModule):
    def setup(self):
        ...
        # 2. Récupérer la sidebar
        sidebar = getattr(self.main_window, 'sidebar', None)
        
        if sidebar and hasattr(sidebar, 'add_menu_button'):
            # ✅ Utiliser la méthode de la sidebar pour ajouter le bouton
            self.btn = sidebar.add_menu_button("Paramètres", "settings", self)
            
            # Définir l'icône via get_icon
            self.btn.setIcon(get_icon('settings', color='#cbd5f5', size=20))
            self.btn.setIconSize(QSize(20, 20))
            ...
            self.btn.setIcon(get_icon('settings', color='#546e7a', size=40))
            self.btn.setIconSize(QSize(40, 40))
            self.btn.setText("  Paramètres")
