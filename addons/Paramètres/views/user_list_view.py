...
class UserListPage(QWidget):
    def __init__(self, controller=None, current_user=None, **kwargs):
        super().__init__()
        self.controller = controller
        self.user = current_user
        print(self.user, self.controller) # DEBUG : Vérifie que tu as bien les infos dans la console
        ...
        # z = getattr(self.user, 'role', 'guest') # Par défaut 'guest' et non 'Admin'
    ...
    def display_users(self, users):
        """Remplit le tableau avec la liste des utilisateurs reçue"""
        # 1. On vide le tableau de cette page
        self.table.setRowCount(0)
        
        # 2. On boucle directement sur les utilisateurs
        for user in users:
            ...
            # Rôle (Utilise ta méthode existante)
            role_value = getattr(user, 'role', 'agent')
            self.set_role_badge(row, role_value)

            # Actions (Utilise ta méthode existante)
