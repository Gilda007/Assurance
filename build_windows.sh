...
compile_with_pyinstaller() {
    ...
    # Lancer PyInstaller via le Python sélectionné et afficher la sortie en temps réel
    "$PYTHON_CMD" -m PyInstaller --noconfirm --onedir $console_opt \
        --name "${build_name}" \
        --icon "icon.ico" \
...
# Vérifier la disponibilité de PyInstaller (soit commande, soit module disponible pour le python choisi)
if ! command -v pyinstaller &> /dev/null && ! "$PYTHON_CMD" -m PyInstaller --version &> /dev/null; then
    echo -e "${YELLOW}⚠ PyInstaller non trouvé pour ${PYTHON_CMD}, installation...${NC}"
    "$PYTHON_CMD" -m pip install --upgrade pip >> "$LOG_FILE" 2>&1
    "$PYTHON_CMD" -m pip install pyinstaller >> "$LOG_FILE" 2>&1
fi

if ! command -v pyinstaller &> /dev/null && ! "$PYTHON_CMD" -m PyInstaller --version &> /dev/null; then
    echo -e "${RED}✗ PyInstaller n'a pas pu être installé pour ${PYTHON_CMD} !${NC}"
    log "ERREUR: PyInstaller non installe pour ${PYTHON_CMD}"
    exit 1
fi

# Afficher la version détectée (préférence pour la commande système si présente)
if command -v pyinstaller &> /dev/null; then
    PYINST_VERSION=$(pyinstaller --version 2>/dev/null || true)
else
    PYINST_VERSION=$($PYTHON_CMD -m PyInstaller --version 2>/dev/null || true)
fi
echo -e "${GREEN}✓ PyInstaller disponible: ${PYINST_VERSION}${NC}"
log "PyInstaller version: ${PYINST_VERSION}"
echo ""
