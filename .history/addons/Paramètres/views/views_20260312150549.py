...
ParametreModuleWidget(self.controller, self.user)                ctrl = self.controller.get('module') if is_dict else ParametreModuleWidget(self.controller, self.user)
                page = ParametreModuleWidget(ctrl, self.user)
                
            elif key == "account":
                # Pour les paramètres de compte
                page = AccountSettingsView(self.user)
