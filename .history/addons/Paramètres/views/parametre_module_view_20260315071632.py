...
class ParametreModuleWidget(QWidget):
    ...
    def setup_ui(self):
        ...
        self.module_list.setStyleSheet("""
            ...
            QListWidget::item { border-bottom: 1px solid #f8fafc; padding: 12px; }
        ...
        self.detail_container.setStyleSheet("background: white; border-radius: 12px; border: 1px solid #e2e8f0;")
        self.detail_layout = QVBoxLayout(self.detail_container)
        
        # On crée un conteneur spécifique pour le contenu dynamique afin de ne pas supprimer le placeholder par erreur
        self.dynamic_content = QWidget()
        self.dynamic_layout = QVBoxLayout(self.dynamic_content)
        self.dynamic_layout.setContentsMargins(10, 10, 10, 10)
        ...
        self.main_layout.addWidget(self.detail_container, 1)
    ...
    def display_module_details(self, index):
        ...
        # Nettoyage du contenu dynamique uniquement
        ...
        author = QLabel(f"Par : {mod['author']} | Version : {mod['version']}")
        author.setStyleSheet("color: #64748b; font-size: 12px; margin-bottom: 10px;")
        self.dynamic_layout.addWidget(author)

        scroll = QScrollArea()
        scroll.setWidgetResizable(True)
        scroll.setStyleSheet("border: 1px solid #f1f5f9; border-radius: 5px;")
        desc = QLabel(mod['description'])
        desc.setWordWrap(True)
        desc.setStyleSheet("padding: 10px; color: #334155;")
        scroll.setWidget(desc)
        self.dynamic_layout.addWidget(scroll, 1)

        deps = QLabel(f"<b>Dépendances :</b> {', '.join(mod.get('depends', [])) or 'Aucune'}")
        deps.setStyleSheet("margin-top: 10px; font-size: 11px;")
        ...
        btn_about.setStyleSheet("background: #f1f5f9; padding: 8px; border-radius: 5px;")
        btn_about.clicked.connect(lambda: QDesktopServices.openUrl(QUrl(mod.get('website', ''))))
        
        is_active = mod.get("installable", True)
        btn_action = QPushButton("Désinstaller" if is_active else "Activer")
        btn_action.setStyleSheet(f"background: {'#ef4444' if is_active else '#10b981'}; color: white; padding: 8px; border-radius: 5px; font-weight: bold;")
        btn_action.clicked.connect(lambda: self.toggle_module(mod['folder_name'], is_active))

        btn_box.addWidget(btn_about)
        btn_box.addStretch()
        btn_box.addWidget(btn_action)
        self.dynamic_layout.addLayout(btn_box)
    ...
    def toggle_module(self, folder, state):
        ...
        try:
            ...
            self.refresh_data()
            QMessageBox.information(self, "Succès", "Le statut du module a été modifié.")
        except Exception as e: QMessageBox.critical(self, "Erreur", str(e))

    def import_zip_module(self):
        path, _ = QFileDialog.getOpenFileName(self, "Importer un ZIP", "", "Zip (*.zip)")
        if not path: return
        try:
            with zipfile.ZipFile(path, 'r') as z:
                z.extractall(os.path.join(os.getcwd(), "addons"))
            self.refresh_data()
            QMessageBox.information(self, "Import", "Module importé avec succès.")
        except Exception as e: QMessageBox.critical(self, "Erreur", str(e))