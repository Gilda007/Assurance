...
class ParametreMainView(QWidget):
    ...
    def display_page(self, row):
        ...
        if not isinstance(self.controller, dict):
            print(f"ERREUR: Controllers est de type {type(self.controller)}, dictionnaire attendu.")
            return
        ...
        if key not in self.pages_cache:
            try:
                if key == "users":
                    ...
                    self.pages_cache[key] = UserListPage(self.controller.get('user'), self.user)
                    
                elif key == "modules":
                    self.pages_cache[key] = ParametreModuleWidget(self.controller.get('module'), self.user)
                    
                elif key == "account":
                    self.pages_cache[key] = AccountSettingsView(self.controller.get('user'), self.user)
