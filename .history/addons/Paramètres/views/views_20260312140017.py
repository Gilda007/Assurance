...
class ParametreMainView(QWidget):
    ...
    def display_users(self, users):
        """Gère l'affichage des utilisateurs en routant vers la bonne sous-page"""
        
        # 1. Si la page n'est pas dans le cache, on la crée proprement
        if "users" not in self.pages_cache:
            print("DEBUG: Création initiale de UserListPage")
            from addons.Paramètres.views.user_list_view import UserListPage
            
            # Récupération du contrôleur
            ctrl = self.controller.get('user') if isinstance(self.controller, dict) else self.controller
            
            # On crée l'instance et on l'ajoute au cache
            # Note : self.user peut être None ici, mais le correctif ci-dessus (getattr) gérera le cas
            self.pages_cache["users"] = UserListPage(ctrl, self.user)
            self.container.addWidget(self.pages_cache["users"])

        # 2. On récupère la page depuis le cache
        user_page = self.pages_cache["users"]
        
        # 3. ON DÉLÈGUE l'affichage à la sous-page
        # C'est UserListPage qui possède le tableau (self.table), pas ParametreMainView !
        if hasattr(user_page, 'display_users'):
            user_page.display_users(users)
        else:
            print("ERREUR : La méthode display_users manque dans UserListPage")
