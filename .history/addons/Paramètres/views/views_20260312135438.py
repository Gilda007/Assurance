...
class ParametreMainView(QWidget):
    ...
    def display_page(self, row):
        ...
        # Si la page n'est pas en cache, on la crée
        if key not in self.pages_cache:
            if key == "users":
                page = UserListPage(self.controller, self.user)
            elif key == "modules":
                page = ParametreModuleWidget(self.controller, self.user)
            elif key == "account":
    ...
    # 1. Vérifier si la page est déjà dans le cache
    if "users" not in self.pages_cache:
        print("DEBUG: Création automatique de la page UserListPage via display_users")
        # On simule un clic ou on appelle directement la création
        # Important : utiliser self.controller (ou le dictionnaire de contrôleurs selon ta structure)
        from addons.Paramètres.views.user_list_view import UserListPage
        
        # On passe le bon contrôleur (vérifie si c'est self.controller ou self.controllers['user'])
    ...
    # 2. Maintenant que nous sommes SURS que la clé existe, on appelle la fonction
    self.pages_cache["users"].display_users(users)

        user_page = self.pages_cache.get("users")
    
    # 2. Si elle existe, on lui demande d'afficher les utilisateurs
        if user_page:
            user_page.display_users(users)
        else:
            # Si la page n'a jamais été ouverte, on la crée forcement avant d'afficher
            self.sidebar.setCurrentRow(0) # Force le chargement
            self.pages_cache["users"].display_users(users)
