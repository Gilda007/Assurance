...
from PySide6.QtCore import Qt, QSize, QUrl # <--- QUrl est ICI
...
class ParametreModuleWidget(QWidget):
    ...
    def setup_ui(self):
        ...
        # --- PANNEAU GAUCHE : LISTE & IMPORT ---
        left_panel = QVBoxLayout()
        
        # Bouton Importation ZIP
        self.btn_import = QPushButton("📥 Importer un module (.zip)")
        ...
        self.module_list.setStyleSheet("""
            ...
            QListWidget::item { border-bottom: 1px solid #f8fafc; padding: 12px; color: #475569; }
            QListWidget::item:selected { background: #eff6ff; color: #3b82f6; border-left: 5px solid #3b82f6; }
        """)
        ...
        self.detail_view = QFrame()
        self.detail_view.setStyleSheet("background: white; border-radius: 12px; border: 1px solid #e2e8f0;")
        self.detail_layout = QVBoxLayout(self.detail_view)
        self.detail_layout.setContentsMargins(30, 30, 30, 30)
        
        self.placeholder = QLabel("Sélectionnez un module pour afficher les détails")
        self.placeholder.setAlignment(Qt.AlignCenter)
        self.placeholder.setStyleSheet("color: #94a3b8; font-size: 15px;")
        self.detail_layout.addWidget(self.placeholder)
        
        self.main_layout.addWidget(self.detail_view, 1)

    def get_all_modules(self):
        """Scanne le dossier addons pour trouver manifest.json"""
        modules = []
        # On remonte d'un niveau si on est dans addons/Paramètres/views/
        base_dir = os.getcwd()
        ...
        for folder in os.listdir(addons_path):
            ...
            if os.path.isdir(folder_path) and not folder.startswith("."):
                # Valeurs par défaut
                mod = {
                    "name": folder, "version": "1.0.0", "author": "Non spécifié",
                    "description": "Aucune description fournie.", "installable": True,
                    "folder_name": folder, "website": "https://google.com", 
                    "depends": [], "category": "Module"
                }
                ...
                if os.path.exists(manifest_path):
                    try:
                        with open(manifest_path, 'r', encoding='utf-8') as f:
                            data = json.load(f)
                            # Correction automatique si description est une liste
    ...
    def display_module_details(self, index):
        ...
        # 1. En-tête (Nom & Entreprise)
        title = QLabel(mod['name'])
        title.setStyleSheet("font-size: 26px; font-weight: 800; color: #1e293b;")
        self.detail_layout.addWidget(title)

        meta = QLabel(f"Version {mod['version']} | Développé par : <b>{mod['author']}</b>")
        meta.setStyleSheet("color: #64748b; font-size: 13px; margin-bottom: 20px;")
        ...
        desc_content = QLabel(mod.get('description', ''))
        desc_content.setWordWrap(True)
        desc_content.setStyleSheet("padding: 15px; color: #334155; font-size: 14px; line-height: 1.5;")
        scroll.setWidget(desc_content)
        self.detail_layout.addWidget(scroll, 1)

        # 3. Dépendances
        deps_box = QHBoxLayout()
        deps_title = QLabel("Dépendances : ")
        deps_title.setStyleSheet("font-weight: bold; color: #475569;")
        deps_box.addWidget(deps_title)
        
        deps_list = mod.get('depends', [])
        deps_label = QLabel(", ".join(deps_list) if deps_list else "Aucune")
        deps_label.setStyleSheet("color: #3b82f6;")
        deps_box.addWidget(deps_label)
        deps_box.addStretch()
        self.detail_layout.addLayout(deps_box)
        ...
        btn_about.setStyleSheet("background: #f1f5f9; color: #1e293b; padding: 10px; border-radius: 6px; font-weight: bold;")
        ...
        btn_uninstall = QPushButton("Désinstaller" if is_installed else "Réactiver")
        btn_uninstall.setFixedWidth(140)
        color = "#ef4444" if is_installed else "#10b981"
        btn_uninstall.setStyleSheet(f"background: {color}; color: white; padding: 10px; border-radius: 6px; font-weight: bold;")
        btn_uninstall.clicked.connect(lambda: self.toggle_module_status(mod['folder_name'], is_installed))
    ...
    def toggle_module_status(self, folder, current_state):
        path = os.path.join(os.getcwd(), "addons", folder, "manifest.json")
        try:
            with open(path, 'r', encoding='utf-8') as f:
                data = json.load(f)
            
            data["installable"] = not current_state
            
            with open(path, 'w', encoding='utf-8') as f:
                json.dump(data, f, indent=4, ensure_ascii=False)
            
            self.refresh_data()
            QMessageBox.information(self, "Système", f"Le module {folder} a été mis à jour.")
        except Exception as e:
            QMessageBox.critical(self, "Erreur", f"Action impossible : {e}")

    def import_zip_module(self):
        file_path, _ = QFileDialog.getOpenFileName(self, "Importer un Module (ZIP)", "", "Modules (*.zip)")
        if not file_path: return

        addons_path = os.path.join(os.getcwd(), "addons")
        try:
            with zipfile.ZipFile(file_path, 'r') as zip_ref:
                # On vérifie si le zip contient bien un manifest.json
                if not any("manifest.json" in f for f in zip_ref.namelist()):
                    QMessageBox.warning(self, "Invalide", "Ce fichier ZIP ne semble pas être un module valide (manifest.json manquant).")
                    return
                
                zip_ref.extractall(addons_path)
            
            self.refresh_data()
            QMessageBox.information(self, "Succès", "Module importé et décompressé avec succès.")
        except Exception as e:
