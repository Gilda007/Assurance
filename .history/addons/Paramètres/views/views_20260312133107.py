...
from addons.Paramètres.views.account_setting_view import AccountSettingsView
...
class ParametreMainView(QWidget):
   def __init__(self, controller=None, user=None):
    ...
    def display_page(self, row):
        ...
        # Si la page n'est pas en cache, on la crée
            if key not in self.pages_cache:
                if key == "users":
                    from addons.Paramètres.views.views import UserListView # Import local
                    page = UserListView(self.controller, self.user)
                elif key == "modules":
    ...
    def get_user_page(self):
        return UserListView(self.controller, self.user)
