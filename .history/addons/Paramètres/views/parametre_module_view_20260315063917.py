...
class ParametreModuleWidget(QWidget):
    ...
    def _create_stat_card(self, title, color):
        """Crée une carte de statistique élégante"""
        card = QFrame()
        ...
        layout = QVBoxLayout(card)
        title_lbl = QLabel(title.upper())
        title_lbl.setStyleSheet("color: #64748b; font-size: 10px; font-weight: 800; letter-spacing: 1px;")
        
        val_lbl = QLabel("--")
        val_lbl.setStyleSheet(f"color: {color}; font-size: 22px; font-weight: 800;")
        
        layout.addWidget(title_lbl)
        layout.addWidget(val_lbl)
        
        # On attache le label de valeur à la carte pour le mettre à jour plus tard
    ...
    def setup_ui(self):
        ...
        self.card_size = self._create_stat_card("Taille Totale", "#f59e0b")
        ...
        self.btn_install.setStyleSheet("""
            QPushButton { 
                background-color: #0f172a; color: white; border-radius: 8px; font-weight: bold;
            }
            QPushButton:hover { background-color: #1e293b; }
        """)
        self.btn_install.clicked.connect(self.on_install_clicked)
        ...
        self.module_list.setObjectName("ModernList")
        self.module_list.setStyleSheet("""
            QListWidget#ModernList { background: transparent; border: none; }
            QListWidget#ModernList::item {
                background: white; border-radius: 8px; padding: 12px; margin-bottom: 5px; border: 1px solid #e2e8f0;
            }
            QListWidget#ModernList::item:selected {
        ...
        # --- CORRECTION ICI : Initialisation du badge ---
        ...
        self.lbl_desc = QLabel("")
        self.lbl_desc.setWordWrap(True)
        self.lbl_desc.setStyleSheet("color: #475569; font-size: 14px; margin-top: 15px; line-height: 150%;")

        self.btn_uninstall = QPushButton("🗑️ Désinstaller")
        self.btn_uninstall.setFixedSize(130, 35)
        self.btn_uninstall.setStyleSheet("""
            QPushButton { background: #fff1f2; color: #e11d48; border-radius: 6px; font-weight: bold; border: 1px solid #fecdd3; }
        ...
        detail_inner.addWidget(self.lbl_name)
        detail_inner.addWidget(self.badge_version)
        detail_inner.addWidget(self.lbl_desc)
        detail_inner.addStretch()
        detail_inner.addWidget(self.btn_uninstall, 0, Qt.AlignRight)
    ...
    def refresh_data(self):
        ...
        self.card_total.val_lbl.setText(str(len(modules)))
        self.card_installed.val_lbl.setText(str(len(modules))) # À adapter selon votre logique
        self.card_size.val_lbl.setText(f"{total_size / 1024:.1f} MB")
    ...
    def on_install_clicked(self):
        path, _ = QFileDialog.getOpenFileName(self, "Installer", "", "Zip (*.zip)")
        if path:
            success, msg = self.controller.install_zip(path)
            if success:
    ...
    def on_uninstall_clicked(self):
        folder = self.btn_uninstall.property("folder")
        if QMessageBox.question(self, "Confirmation", f"Supprimer {folder} ?") == QMessageBox.Yes:
