...
from PySide6.QtGui import QColor, QFont
import os
import json # Importation nécessaire pour lire le manifest.json
...
class ParametreModuleWidget(QWidget):
    ...
    def setup_ui(self):
        ...
        title_list = QLabel("Composants du Système (Scan addons/)")
        title_list.setStyleSheet("font-size: 18px; font-weight: bold; color: #1e293b; border: none; padding: 10px;")
        list_vbox.addWidget(title_list)
    ...
    def get_all_module(self):
        """
        Scanne le dossier 'addons' et lit les fichiers 'manifest.json'.
        """
        modules = []
        # Localisation du dossier addons à la racine du projet
        addons_path = os.path.join(os.getcwd(), "addons")
        ...
        try:
            for folder in os.listdir(addons_path):
                ...
                if os.path.isdir(folder_path) and not folder.startswith(("__", ".")):
                    # CHANGER : On cherche manifest.json
                    json_path = os.path.join(folder_path, "manifest.json")
                    
                    # Valeurs par défaut si le JSON est absent ou corrompu
                    module_info = {
                        "name": folder.replace("_", " ").title(),
                        "version": "1.0.0",
                        "author": "Inconnu",
                        "description": f"Dossier : {folder}",
                        "folder_name": folder,
                        "size": self._get_folder_size(folder_path)
                    }
                    ...
                    if os.path.exists(json_path):
                        try:
                            with open(json_path, "r", encoding="utf-8") as f:
                                # Utilisation de json.load() au lieu de eval()
                                manifest_data = json.load(f)
                                if isinstance(manifest_data, dict):
                                    module_info.update(manifest_data)
                        except Exception as e:
                            print(f"Erreur lors de la lecture du JSON dans {folder}: {e}")
    ...
    def refresh_data(self):
        ...
        self.card_total.findChild(QLabel, "value").setText(str(len(modules)))
        self.card_installed.findChild(QLabel, "value").setText(str(len(modules)))
        self.card_storage.findChild(QLabel, "value").setText(f"{total_size / (1024*1024):.2f} MB")
    ...
    def _create_module_item_widget(self, mod):
        ...
        icon_label = QLabel("🧩")
        icon_label.setStyleSheet("font-size: 22px; background: #f1f5f9; border-radius: 8px; padding: 5px;")
        
        info_layout = QVBoxLayout()
        name_label = QLabel(f"{mod.get('name')} <span style='color: #94a3b8; font-size: 10px;'>{mod.get('version')}</span>")
        name_label.setStyleSheet("font-weight: bold; font-size: 14px; color: #1e293b;")
        
        # Tronquer la description si elle est trop longue pour l'affichage
        desc_text = mod.get('description', 'Aucune description.')
        if len(desc_text) > 85: desc_text = desc_text[:82] + "..."
        ...
        # Statut (Catégorie du JSON)
        cat_label = QLabel(mod.get('category', 'Module'))
        cat_label.setStyleSheet("color: #3b82f6; background: #eff6ff; padding: 4px 10px; border-radius: 5px; font-size: 10px; font-weight: bold;")
        layout.addWidget(cat_label)
        
        btn_action = QPushButton("Ouvrir")
        btn_action.setFixedWidth(70)
        btn_action.setCursor(Qt.PointingHandCursor)
        btn_action.setStyleSheet("""
            QPushButton {
                background-color: white; color: #1e293b; border-radius: 6px; 
                padding: 6px; font-weight: bold; border: 1px solid #e2e8f0;
            }
