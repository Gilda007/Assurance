...
class ParametreMainView(QWidget):
    ...
    def display_page(self, row):
        ...
        if key not in self.pages_cache:
            if key == "users":
                ...
                module_ctrl = ParametreModuleWidget() 
                new_page = ParametreModuleWidget(module_ctrl, self.user)
            elif key == "account":
    ...
    # 1. On s'assure que la page existe dans le cache
        ...
        # 2. On récupère la page et on lui donne les données
        user_page = self.pages_cache["users"]
        user_page.display_users(users) # Appelle la méthode corrigée ci-dessus
