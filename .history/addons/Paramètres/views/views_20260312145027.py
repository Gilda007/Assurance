...
class ParametreMainView(QWidget):
    ...
    def display_page(self, row):
        ...
        if key not in self.pages_cache:
            ...
            if key == "users":
                ...
                # Si on n'a pas de dictionnaire, on importe et crée le contrôleur de module ici
                ctrl = self.controller.get('module') if is_dict else ParametreModuleWidget()
                page = ParametreModuleWidget(ctrl, self.user)
    ...
    def display_users(self, users):
        """Affiche les utilisateurs sans faire planter la navigation"""
        # 1. On s'assure que la page 'users' est créée dans le cache
        if "users" not in self.pages_cache:
            # On cherche l'index de la sidebar qui correspond aux utilisateurs (souvent 0)
            self.display_page(0) 

        # 2. On récupère la page et on lui envoie les données
