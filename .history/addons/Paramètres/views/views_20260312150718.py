    ...
    def display_page(self, row):
        ...
        if key not in self.pages_cache:
            ...
            if key == "users":
                ...
                # 1. On récupère ou on crée le CONTRÔLEUR
                # On passe la session si elle existe
                session = getattr(self.controller, 'session', None)
                ctrl = self.controller.get('module') if isinstance(self.controller, dict) else ModuleController(session)
                
                # 2. On crée la VUE en lui donnant ce contrôleur
