...
class ParametreModuleWidget(QWidget):
    def __init__(self, controller=None, user=None):
        ...
        # Initialisation des attributs de liste pour éviter l'AttributeError
        self.list_installed = None
        self.list_available = None
    ...
    def setup_ui(self):
        ...
        self.card_total = self._create_card("Détectés", "#3b82f6")
        self.card_active = self._create_card("Installés", "#10b981")
        ...
        # CRUCIAL : On définit les noms exacts ici
        self.list_installed = QListWidget()
        self.list_available = QListWidget()
        
        self.tabs.addTab(self.list_installed, "Modules Installés")
        self.tabs.addTab(self.list_available, "Modules Disponibles")
        
        self.main_layout.addWidget(self.tabs)

    def _create_card(self, title, color):
        card = QFrame()
        card.setStyleSheet(f"background: white; border-radius: 12px; border-left: 6px solid {color}; border: 1px solid #e2e8f0;")
        card.setFixedHeight(90)
        l = QVBoxLayout(card)
        t = QLabel(title); t.setStyleSheet("color: #64748b; font-size: 11px; font-weight: bold; border:none;")
        v = QLabel("0"); v.setObjectName("val"); v.setStyleSheet(f"color: {color}; font-size: 22px; font-weight: bold; border:none;")
        l.addWidget(t); l.addWidget(v)
        return card

    def get_modules_from_disk(self):
        """Lit tous les dossiers dans addons/ et parse manifest.json"""
        modules = []
                ...
                if os.path.exists(manifest_path):
                    try:
                        with open(manifest_path, 'r', encoding='utf-8') as f:
                            ...
                            # Correction du bug de type (liste vers string pour QLabel)
                            if "description" in raw_data and isinstance(raw_data["description"], list):
                                raw_data["description"] = " ".join(raw_data["description"])
                            m_data.update(raw_data)
                    except Exception as e:
                        print(f"Erreur lecture JSON {folder}: {e}")
                
                modules.append(m_data)
        return modules

    def toggle_module_activation(self, folder_name, current_install_status):
        """Action du bouton : modifie 'installable' dans le fichier manifest.json"""
        manifest_path = os.path.join(os.getcwd(), "addons", folder_name, "manifest.json")
        ...
        try:
            ...
            # On bascule l'état : si c'était vrai, ça devient faux.
            data["installable"] = not current_install_status
            ...
            QMessageBox.critical(self, "Erreur de fichier", f"Impossible d'écrire dans {folder_name} :\n{str(e)}")

    def refresh_data(self):
        """Remplit les listes et met à jour les stats"""
        if not self.list_installed or not self.list_available:
            return

        self.list_installed.clear()
        self.list_available.clear()
        
        all_mods = self.get_modules_from_disk()
        installed_count = 0
        ...
        for mod in all_mods:
            ...
            # Création du widget personnalisé pour la ligne
            row_widget = QWidget()
            ...
            name_lbl = QLabel(f"{mod.get('name')} (v{mod.get('version')})")
            name_lbl.setStyleSheet(f"font-weight: bold; color: {'#1e293b' if is_active else '#94a3b8'}; font-size: 14px;")
            ...
            btn_style = "background: #ef4444;" if is_active else "background: #10b981;"
            btn.setStyleSheet(f"{btn_style} color: white; font-weight: bold; border-radius: 6px; padding: 7px;")
            
            # Connection du clic
            btn.clicked.connect(lambda checked=False, f=mod['folder_name'], s=is_active: 
                                self.toggle_module_activation(f, s))
            
            layout.addWidget(btn)
            target_list.setItemWidget(item, row_widget)

        # Mise à jour des compteurs visuels
        self.card_total.findChild(QLabel, "val").setText(str(len(all_mods)))
        self.card_active.findChild(QLabel, "val").setText(str(installed_count))
    ...
    def _get_size(self, path):
        ...
        try:
            for entry in os.scandir(path):
                if entry.is_file(): total += entry.stat().st_size
                elif entry.is_dir(): total += self._get_size(entry.path)
        except: pass
        return total