...
class ParametreModuleWidget(QWidget):
    ...
    def get_all_modules(self):
        ...
        for folder in os.listdir(addons_path):
            ...
            if os.path.isdir(folder_path) and not folder.startswith("."):
                mod = {"name": folder, "installable": False, "folder_name": folder, "description": ""}
                
                if os.path.exists(json_path):
                    try:
                        with open(json_path, 'r', encoding='utf-8') as f:
                            data = json.load(f)
                            # --- CORRECTION DU BUG : Conversion liste -> string ---
    ...
    def toggle_module(self, folder_name, current_state):
        """Désinstalle (is_active = None/False) ou Installe"""
        path = os.path.join(os.getcwd(), "addons", folder_name, "manifest.json")
