...
class ParametreModuleWidget(QWidget):
    def __init__(self, controller, user=None):
        super().__init__()
        self.controller = controller
        self.service = controller # Pour la compatibilité avec vos appels
        self.user = user
        self.setup_ui()
        self.refresh_data()
    ...
    def _create_stat_card(self, title, color):
        card = QFrame()
        card.setFixedHeight(100)
        card.setStyleSheet(f"background: white; border-radius: 12px; border-left: 5px solid {color};")
        
        layout = QVBoxLayout(card)
        t_lbl = QLabel(title.upper())
        t_lbl.setStyleSheet("color: #64748b; font-size: 10px; font-weight: bold;")
        ...
        # CRUCIAL : Utilisez le même nom de variable ici et dans refresh_data
    ...
    def refresh_data(self):
        ...
        # Mise à jour des cartes (Utilisation de value_label)
        self.card_total.value_label.setText(str(len(modules)))
        self.card_installed.value_label.setText(str(len(modules)))
        self.card_storage.value_label.setText(f"{total_size / 1024:.1f} MB")
    ...
    def display_details(self, row):
        ...
        self.lbl_name.setText(mod['name'])
        self.lbl_size_tag.setText(f"TAILLE : {mod.get('size', 0)} KB")
        self.lbl_desc.setText(mod.get('description', 'Aucune description.'))
        
        # Correction : vérifiez si badge_version existe avant de l'utiliser
        if hasattr(self, 'badge_version'):
    ...
    # Supprimez les doublons de display_details et refresh_list en bas du fichier !