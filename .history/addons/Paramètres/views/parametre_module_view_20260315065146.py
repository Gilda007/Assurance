...
class ParametreModuleWidget(QWidget):
    ...
    def refresh_data(self):
        ...
        for mod in modules:
            ...
            # Info
            info_v = QVBoxLayout()
            name = QLabel(f"{mod.get('name')} ({mod.get('version')})")
            name.setStyleSheet(f"font-weight: bold; color: {'#1e293b' if is_active else '#94a3b8'};")
            desc = QLabel(mod.get('description', '...'))
            desc.setStyleSheet("color: #64748b; font-size: 11px;")
            info_v.addWidget(name); info_v.addWidget(desc)
            
            l.addLayout(info_v, 1)

            # Bouton Désinstaller / Activer
            btn_text = "Désactiver" if is_active else "Activer"
            btn_color = "#ef4444" if is_active else "#10b981"
            
            btn = QPushButton(btn_text)
            btn.setFixedWidth(100)
            btn.setStyleSheet(f"background: {btn_color}; color: white; border-radius: 5px; font-weight: bold; padding: 5px;")
            # On passe le nom du dossier au clic
            btn.clicked.connect(lambda checked=False, f=mod['folder_name']: self.toggle_module_status(f))
            
            l.addWidget(btn)
            self.module_list.setItemWidget(item, w)

        # Update Stats
        self.card_total.findChild(QLabel, "val").setText(str(len(modules)))
        self.card_installed.findChild(QLabel, "val").setText(str(active_count))
        self.card_storage.findChild(QLabel, "val").setText(f"{total_size/(1024*1024):.1f} MB")
