...
class ParametreMainView(QWidget):
    ...
    # 1. Si la page n'est pas dans le cache, on la crée
    if "users" not in self.pages_cache:
        print("DEBUG: Création automatique de la page UserListPage")
        from addons.Paramètres.views.user_list_view import UserListPage
        
        # On récupère le bon contrôleur
        ctrl = self.controller.get('user') if isinstance(self.controller, dict) else self.controller
        
        # Sécurité : vérifier que l'utilisateur n'est pas None
        if self.user is None:
            print("ERREUR: self.user est None, impossible de vérifier les rôles.")
            # On peut passer un objet vide ou gérer l'erreur
    ...
    # 3. On demande à LA PAGE de remplir son propre tableau
    # On ne fait plus de self.table ici !
