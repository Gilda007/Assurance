...
class ParametreModuleWidget(QWidget):
    ...
    def setup_ui(self):
        # Style global - Fond gris très clair pour faire ressortir les cartes blanches
        self.setStyleSheet("background-color: #f1f5f9; border: none;")
        self.main_layout = QVBoxLayout(self)
        self.main_layout.setContentsMargins(30, 30, 30, 30)
        self.main_layout.setSpacing(25)

        # --- 1. BARRE DE TITRE & IMPORT ---
        top_bar = QHBoxLayout()
        header_title = QLabel("Gestion des Composants")
        header_title.setStyleSheet("font-size: 24px; font-weight: 800; color: #1e293b;")
        
        btn_import = QPushButton("📥 Importer ZIP")
        btn_import.setCursor(Qt.PointingHandCursor)
        btn_import.setStyleSheet("""
            QPushButton { 
                background: #3b82f6; color: white; font-weight: bold; border-radius: 8px; 
        ...
        self.stats_layout = QHBoxLayout()
        self.card_total = self._create_stat_card("Modules Disponibles", "0", "#64748b")
        self.card_installed = self._create_stat_card("Modules Installés", "0", "#10b981")
        self.card_inactive = self._create_stat_card("Modules Inactifs", "0", "#f59e0b")
        
        self.stats_layout.addWidget(self.card_total)
        self.stats_layout.addWidget(self.card_installed)
        self.stats_layout.addWidget(self.card_inactive)
        self.main_layout.addLayout(self.stats_layout)

        # --- 3. ZONE PRINCIPALE (LISTE | DETAILS) ---
        ...
        self.module_list.setStyleSheet("""
            QListWidget { 
                background: white; border-radius: 12px; border: 1px solid #e2e8f0; outline: none; 
            }
            QListWidget::item { 
                padding: 15px; border-bottom: 1px solid #f1f5f9; color: #475569;
            }
            QListWidget::item:selected { 
                background: #f8fafc; color: #3b82f6; border-left: 5px solid #3b82f6; font-weight: bold;
            }
        """)
        ...
        self.detail_frame.setStyleSheet("background: white; border-radius: 12px; border: 1px solid #e2e8f0;")
        self._add_shadow(self.detail_frame)
        self.detail_layout = QVBoxLayout(self.detail_frame)
        self.detail_layout.setContentsMargins(0, 0, 0, 0) # Sera rempli par dynamic_content
        ...
        self.placeholder = QLabel("Sélectionnez un module pour explorer ses fonctionnalités")
        self.placeholder.setAlignment(Qt.AlignCenter)
        self.placeholder.setStyleSheet("color: #94a3b8; font-size: 15px;")
        
        self.detail_layout.addWidget(self.placeholder)
        self.detail_layout.addWidget(self.dynamic_content)
        self.dynamic_content.hide()

        self.content_layout.addWidget(self.module_list)
        self.content_layout.addWidget(self.detail_frame, 1)
        self.main_layout.addLayout(self.content_layout)

    def _create_stat_card(self, title, value, color):
        card = QFrame()
        card.setFixedHeight(100)
        card.setStyleSheet(f"background: white; border-radius: 12px; border-bottom: 4px solid {color};")
        self._add_shadow(card)
        
        layout = QVBoxLayout(card)
        t = QLabel(title)
        t.setStyleSheet("color: #64748b; font-size: 12px; font-weight: bold; text-transform: uppercase;")
        v = QLabel(value)
        v.setObjectName("val")
        v.setStyleSheet(f"color: #1e293b; font-size: 28px; font-weight: 800;")
    ...
    def get_all_modules(self):
        ...
        for folder in os.listdir(addons_path):
            ...
            if os.path.isdir(folder_path) and not folder.startswith("."):
                mod = {"name": folder, "version": "1.0", "author": "Inconnu", "description": "", 
                       "installable": True, "folder_name": folder, "website": "", "depends": []}
                if os.path.exists(manifest_path):
                    try:
                        with open(manifest_path, 'r', encoding='utf-8') as f:
                            data = json.load(f)
                            if isinstance(data.get("description"), list): data["description"] = "\n".join(data["description"])
                            mod.update(data)
                    except: pass
                modules.append(mod)
        return modules
    ...
    def refresh_data(self):
        self.module_list.clear()
        self.current_modules = self.get_all_modules()
        
        installed = 0
        for mod in self.current_modules:
            if mod.get('installable'): installed += 1
            item = QListWidgetItem(mod['name'])
            item.setData(Qt.UserRole, mod)
            self.module_list.addItem(item)
            
        # Update Cards
        self.card_total.findChild(QLabel, "val").setText(str(len(self.current_modules)))
        self.card_installed.findChild(QLabel, "val").setText(str(installed))
        self.card_inactive.findChild(QLabel, "val").setText(str(len(self.current_modules) - installed))

    def display_module_details(self, index):
        if index < 0: return
        self.placeholder.hide()
        self.dynamic_content.show()
        ...
        ver.setStyleSheet("background: #f1f5f9; color: #64748b; padding: 4px 8px; border-radius: 6px; font-size: 11px;")
        h_layout.addWidget(title)
        h_layout.addWidget(ver)
        h_layout.addStretch()
        self.dynamic_layout.addLayout(h_layout)

        author = QLabel(f"Éditeur : {mod['author']}")
        author.setStyleSheet("color: #94a3b8; font-size: 13px; margin-bottom: 15px;")
        self.dynamic_layout.addWidget(author)

        # Desc
        scroll = QScrollArea()
        scroll.setWidgetResizable(True)
        scroll.setStyleSheet("border: none; background: #fdfdfd; border-radius: 8px;")
        desc = QLabel(mod['description'] or "Aucune description détaillée.")
        desc.setWordWrap(True)
        desc.setStyleSheet("padding: 15px; color: #475569; line-height: 140%; font-size: 14px;")
        scroll.setWidget(desc)
        self.dynamic_layout.addWidget(scroll, 1)

        # Footer Actions
        self.dynamic_layout.addWidget(QLabel(f"<b>Dépendances :</b> {', '.join(mod.get('depends',[])) or 'Aucune'}"))
        
        footer = QHBoxLayout()
        btn_web = QPushButton("🌐 About Us")
        btn_web.setCursor(Qt.PointingHandCursor)
        btn_web.setStyleSheet("background: #f1f5f9; color: #1e293b; padding: 10px 15px; border-radius: 8px; font-weight: 600;")
        btn_web.clicked.connect(lambda: QDesktopServices.openUrl(QUrl(mod.get('website', ''))))
        
        is_act = mod.get('installable')
        btn_action = QPushButton("Désinstaller" if is_act else "Activer")
        btn_action.setCursor(Qt.PointingHandCursor)
        btn_action.setStyleSheet(f"background: {'#ef4444' if is_act else '#10b981'}; color: white; padding: 10px 20px; border-radius: 8px; font-weight: 600;")
        btn_action.clicked.connect(lambda: self.toggle_module(mod['folder_name'], is_act))

        footer.addWidget(btn_web)
        footer.addStretch()
        footer.addWidget(btn_action)
        self.dynamic_layout.addLayout(footer)

    def toggle_module(self, folder, state):
        path = os.path.join(os.getcwd(), "addons", folder, "manifest.json")
        try:
            with open(path, 'r') as f: data = json.load(f)
            data["installable"] = not state
            with open(path, 'w') as f: json.dump(data, f, indent=4)
            self.refresh_data()
        except Exception as e: QMessageBox.critical(self, "Erreur", str(e))

    def import_zip_module(self):
        path, _ = QFileDialog.getOpenFileName(self, "Sélectionner le pack ZIP", "", "Zip (*.zip)")
        if not path: return
        try:
            with zipfile.ZipFile(path, 'r') as z:
                z.extractall(os.path.join(os.getcwd(), "addons"))
            self.refresh_data()
            QMessageBox.information(self, "Importation", "Module installé avec succès !")
        except Exception as e: QMessageBox.critical(self, "Erreur ZIP", str(e))