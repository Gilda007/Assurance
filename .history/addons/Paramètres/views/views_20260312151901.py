...
class ParametreMainView(QWidget):
    ...
    def display_page(self, row):
        ...
        if key not in self.pages_cache:
            ...
            # On vérifie si self.user est valide
            if self.user is not None:
                page = AccountSettingsView(self.controller, self.user)
            else:
                # On crée une page temporaire avec un message d'erreur au lieu de crash
                page = QWidget()
                error_layout = QVBoxLayout(page)
                error_msg = QLabel("Impossible d'accéder aux paramètres :\nAucun utilisateur n'est connecté à la session.")
                error_msg.setAlignment(Qt.AlignCenter)
                error_msg.setStyleSheet("color: #d63031; font-weight: bold; font-size: 14px;")
