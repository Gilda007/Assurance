...
class ParametreModuleWidget(QWidget):
    ...
    def setup_ui(self):
        ...
        # --- PARTIE GAUCHE : LISTE ET IMPORT ---
        left_panel = QVBoxLayout()
        
        btn_import = QPushButton("📥 Importer un module (ZIP)")
        btn_import.setStyleSheet("""
            QPushButton { background: #3b82f6; color: white; font-weight: bold; border-radius: 8px; padding: 12px; }
        ...
        self.module_list = QListWidget()
        self.module_list.setFixedWidth(300)
        self.module_list.setStyleSheet("""
            QListWidget { background: white; border-radius: 10px; border: 1px solid #e2e8f0; outline: none; }
            QListWidget::item { border-bottom: 1px solid #f1f5f9; padding: 10px; }
            QListWidget::item:selected { background: #eff6ff; color: #3b82f6; border-left: 4px solid #3b82f6; }
        """)
        ...
        # Widget par défaut quand rien n'est sélectionné
        self.empty_label = QLabel("Sélectionnez un module pour voir les détails")
    ...
    def get_all_modules(self):
        ...
        for folder in os.listdir(addons_path):
            ...
            if os.path.isdir(folder_path) and not folder.startswith("."):
                mod = {
                    "name": folder, "version": "1.0", "author": "Inconnu",
                    "description": "Aucune description.", "installable": True,
                    "folder_name": folder, "website": "https://google.com", "depends": []
                }
                if os.path.exists(json_path):
                    try:
                        with open(json_path, 'r', encoding='utf-8') as f:
                            data = json.load(f)
                            # Correction du bug list/str pour la description
    ...
    def display_module_details(self, index):
        ...
        # --- HEADER ---
        header = QLabel(mod['name'])
        header.setStyleSheet("font-size: 24px; font-weight: bold; color: #1e293b;")
        self.detail_layout.addWidget(header)

        info = QLabel(f"Version {mod['version']} | Par {mod['author']}")
        info.setStyleSheet("color: #64748b; font-size: 13px; margin-bottom: 10px;")
        self.detail_layout.addWidget(info)

        # --- DESCRIPTION ---
        desc_box = QScrollArea()
        desc_box.setWidgetResizable(True)
        desc_lbl = QLabel(mod['description'])
        desc_lbl.setWordWrap(True)
        desc_lbl.setStyleSheet("color: #334155; font-size: 14px; padding: 10px;")
        desc_box.setWidget(desc_lbl)
        self.detail_layout.addWidget(desc_box, 1)

        # --- DEPENDANCES ---
        deps_title = QLabel("Dépendances :")
        deps_title.setStyleSheet("font-weight: bold; margin-top: 10px;")
        self.detail_layout.addWidget(deps_title)
        
        deps = mod.get("depends", [])
        deps_txt = ", ".join(deps) if deps else "Aucune"
        self.detail_layout.addWidget(QLabel(deps_txt))

        # --- BOUTONS ACTIONS ---
        btn_layout = QHBoxLayout()
        
        # About Us
        btn_web = QPushButton("🌐 Site Web")
        btn_web.setStyleSheet("background: #f1f5f9; padding: 10px; border-radius: 6px;")
        btn_web.clicked.connect(lambda: QDesktopServices.openUrl(QUrl(mod.get('website', ''))))
        
        # Désinstaller / Activer
        is_active = mod.get("installable", True)
        btn_toggle = QPushButton("Désactiver" if is_active else "Activer")
        btn_toggle.setStyleSheet(f"background: {'#ef4444' if is_active else '#10b981'}; color: white; padding: 10px; border-radius: 6px; font-weight: bold;")
        btn_toggle.clicked.connect(lambda: self.toggle_module(mod['folder_name'], is_active))
    ...
    def toggle_module(self, folder, current_state):
    ...
    def import_zip_module(self):
        file_path, _ = QFileDialog.getOpenFileName(self, "Importer un Module (ZIP)", "", "Zip Files (*.zip)")
        if not file_path: return

        addons_path = os.path.join(os.getcwd(), "addons")
        try:
            with zipfile.ZipFile(file_path, 'r') as zip_ref:
                # On extrait tout dans le dossier addons
                zip_ref.extractall(addons_path)
            
            self.refresh_data()
            QMessageBox.information(self, "Importation", "Module importé avec succès dans le dossier addons.")
        except Exception as e:
            QMessageBox.critical(self, "Erreur", f"Échec de l'importation : {str(e)}")