...
class AuditVehicleLog(Base):
    ...
    user_id = Column(Integer, ForeignKey('utilisateurs.id')) # Qui
    action = Column(String(50))      # CREATE, UPDATE, DELETE, LOGIN
    module = Column(String(50))      # CONTACTS, CONTRACTS, VEHICLES, FLEETS
    item_id = Column(Integer)        # ID de l'élément modifié
    old_values = Column(Text)        # Valeurs avant (format JSON string)
    new_values = Column(Text)        # Valeurs après (format JSON string)
    ip_public = Column(String(45)) # L'IP de la box internet
    ip_local = Column(String(45))  # L'IP de l'ordinateur (192.168...)
