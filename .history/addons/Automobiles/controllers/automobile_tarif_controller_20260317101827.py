...
class TarifController:
    ...
    def import_tarifs_from_file(self, file_path, user_id):
        ...
        try:
            ...
            # --- CORRECTION ICI : Vérifier les colonnes réelles du fichier tarif ---
            required_columns = ['Cie', 'Tarif', 'Lib_Tarif', 'Prime1']
            for col in required_columns:
                if col not in df.columns:
                    # C'est ici que l'ancienne erreur se produisait
                    return False, f"La colonne obligatoire '{col}' est manquante dans le fichier de tarifs."
            ...
            def to_f(val): 
                try:
                    ...
                    # Gérer les nombres formattés avec des virgules si nécessaire
            ...
            for index, row in df.iterrows():
                try:
                    ...
                    if pd.isna(row.get('Cie')) and pd.isna(row.get('Tarif')):
                        continue
            ...
            self.db.commit()
            return True, f"{success_count} tarifs importés avec succès ({len(errors)} erreurs)."
