...
from addons.Automobiles.models. import AutomobileTarif  # Assurez-vous du chemin d'import
from core.utils.alerts import AlertManager
...
class TarifController:
    ...
    def import_tarifs_from_file(self, file_path, user_id):
        """
        Importation massive depuis le fichier CSV/Excel fourni.
        Gère les colonnes spécifiques : Prime, Remorq, Inflamble, Essence, Diesel.
        """
        try:
            # 1. Lecture du fichier
            # Si c'est le CSV que vous avez envoyé, on utilise read_csv
            ...
            # 2. Fonctions de nettoyage locales
            def to_f(val): # Convertit en Float propre
                try:
                    if pd.isna(val) or str(val).strip() == "": return 0.0
                    return float(val)
                except: return 0.0

            def to_i(val): # Convertit en Int propre
                try:
            ...
            for index, row in df.iterrows():
                try:
                    ...
                    # Commit par paquets de 200 pour la rapidité
                    if success_count % 200 == 0:
                        self.db.commit()

                except Exception as line_error:
                    errors.append(f"Erreur ligne {index}: {str(line_error)}")
                    continue

            self.db.commit()
            return True, f"{success_count} tarifs importés. {len(errors)} erreurs."
