...
from addons.Automobiles.models.compagnie import Compagnie  # Assurez-vous du chemin d'import
...
class CompagnieController:
    ...
    def delete_compagnie(self, compagnie_id):
        """Supprime une compagnie (les tarifs liés seront supprimés par cascade)."""
        try:
            cie = self.db.query(Compagnie).filter(Compagnie.id == compagnie_id).first()
            if cie:
                self.db.delete(cie)
    ...
    def get_all_compagnies(self):
        ...
        return self.db.query(Compagnie).order_by(Compagnie.nom).all()

    def get_compagnie_by_code(self, code):
        """Recherche une compagnie par son code unique."""
        return self.db.query(Compagnie).filter(Compagnie.code == code).first()