            ...
            for index, row in df.iterrows():
                try:
                    ...
                    new_tarif = AutomobileTarif(
                        ...
                        # Vérifiez bien ici les noms exacts (created_at / created_by)
                        created_by=user_id,
                        created_at=datetime.now(),
                        is_active=True,
                        # ... ajoutez vos autres colonnes (prime1, etc.) ici
                        prime1=float(str(row.get('Prime1', 0)).replace(',', '.')) if pd.notna(row.get('Prime1')) else 0.0
                    )
