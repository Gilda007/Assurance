...
class TarifController:
    ...
    #         required_columns = [
    #             "code", "Nom_Cie", "Tarif", "Lib_Tarif", "Categorie", "Zone", 
    ...
    def import_from_excel(self, file_path, user_id, progress_callback=None):
        print(f"\n>>> DEBUG: Début de l'importation")
        print(f">>> DEBUG: Fichier cible: {file_path}")
        print(f">>> DEBUG: Utilisateur ID: {user_id}")
        ...
        try:
            ...
            print(f">>> DEBUG: Pandas a lu {len(df)} lignes.")
            
            if df.empty:
                print(">>> DEBUG: Le DataFrame est vide !")
                return False, "Le fichier est vide."

            success_count = 0
            for index, row in df.iterrows():
                try:
                    # On tente de créer l'objet
                    new_tarif = AutomobileTarif(
                        cie_id=int(float(row.get('Cie', 0))),
                        tarif_code=str(row.get('Tarif', '')),
                        lib_tarif=str(row.get('Lib_Tarif', 'Sans nom')),
                        prime1=float(str(row.get('Prime1', 0)).replace(',', '.')),
                        create_by=user_id,
                        create_at=datetime.now(),
                        is_active=True
                    )
                    ...
                    # Forcer l'appel de la barre de progression
                    ...
                    if success_count % 50 == 0:
                        self.db.commit()
                        print(f">>> DEBUG: 50 lignes enregistrées (Total: {success_count})")

                except Exception as line_err:
                    print(f">>> DEBUG: Erreur à la ligne {index}: {line_err}")
                    continue

            self.db.commit()
            print(f">>> DEBUG: Importation terminée avec succès ({success_count} lignes).")
            return True, f"Importation réussie : {success_count} tarifs ajoutés."

        except Exception as e:
            print(f">>> DEBUG: ERREUR CRITIQUE DANS LE CONTRÔLEUR: {str(e)}")
            self.db.rollback()
