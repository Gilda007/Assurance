...
from addons.Automobiles.models.compagnie import Compagnie  # Assurez-vous que le chemin est correct
    ...
    def create_compagnie(self, form_data, user_login):
        ...
        try:
            ...
            nouvelle_cie = Compagnie(
                ...
                # --- Données d'audit (Automatique) ---
                create_by=audit['user'],
                modify_by=audit['user'],
            ...
            self.db.commit()
            self.logger.info(f"Compagnie {nouvelle_cie.nom} créée par {user_login} (IP: {audit['local_ip']})")
