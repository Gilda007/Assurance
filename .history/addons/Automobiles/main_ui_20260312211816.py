...
class AutomobileModule(BaseModule):
    ...
    def activate_module(self):
        # 1. On tente de récupérer l'utilisateur via la fenêtre principale
        # Vérifiez bien si votre variable s'appelle 'current_user' ou 'user' dans MainWindow
        current_user = getattr(self.main_window, 'current_user', None)

        # DEBUG : Ajoutez cette ligne pour voir ce qui se passe dans votre console
        ...
        # 2. Si on arrive ici, l'utilisateur existe !
        if not hasattr(self, 'view'):
            # On passe bien l'utilisateur à la vue
            self.view = self.create_module_view() 
            self.main_window.stackedWidget.container.addWidget(self.view)

        self.main_window.stackedWidget.container.setCurrentWidget(self.view)
