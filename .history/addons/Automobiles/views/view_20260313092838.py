...
class VehicleMainView(QWidget):
    ...
    def setup_ui(self):
        ...
        self.btn_fleets.clicked.connect(lambda: self._switch_page(1, self.btn_fleets))
        self.btn_clients.clicked.connect(lambda: self._switch_page(1, self.btn_clients))        
    ...
    def _init_vehicles_page(self):
        ...
        # Barre de recherche pour le tableau
        # search_frame = QFrame()
        # search_frame.setStyleSheet("background: white; border-radius: 10px;")
        # search_layout = QHBoxLayout(search_frame)
        # self.search_input = QLineEdit()
        # self.search_input.setPlaceholderText("Rechercher un véhicule...")
        # self.search_input.setStyleSheet("border: none; padding: 10px;")
        # search_layout.addWidget(self.search_input)
        
        # # Tableau
        # self.table = QTableWidget()
        # self.table.setColumnCount(5)
        # self.table.setHorizontalHeaderLabels(["Immat", "Modèle", "Propriétaire", "Statut", "Action"])
        # self.table.horizontalHeader().setSectionResizeMode(QHeaderView.Stretch)
        ...
        # On instancie votre classe complexe 'VehiculeModuleView'
        # On lui passe le contrôleur et l'utilisateur actuel (self.user)
