...
class CompanyTarifForm(QDialog):
    ...
    # 1. Initialiser le dictionnaire avec les champs fixes
    new_tarif_data = {
        'cie_id': self.combo_cie.currentData(),
        'tarif_code': self.txt_code.text(),
        'lib_tarif': self.txt_libelle.text(),
        'created_by': self.current_user.id,
        'created_at': datetime.now(),
        'is_active': True
    }

    # 2. Récupérer dynamiquement les 10 colonnes de chaque catégorie
    # On boucle de 1 à 10 pour remplir les séries de champs
    for i in range(1, 11):
        # Récupération des Primes RC (ex: self.txt_prime1, self.txt_prime2...)
        attr_prime = getattr(self, f"txt_prime{i}", None)
        if attr_prime:
            new_tarif_data[f'prime{i}'] = float(attr_prime.text() or 0)

        # Récupération des Essences (ex: self.txt_essence_1...)
        attr_ess = getattr(self, f"txt_essence_{i}", None)
        if attr_ess:
            new_tarif_data[f'essence_{i}'] = float(attr_ess.text() or 0)

        # Récupération des Diesels
        attr_die = getattr(self, f"txt_diesel_{i}", None)
        if attr_die:
            new_tarif_data[f'diesel_{i}'] = float(attr_die.text() or 0)

        # Récupération des Remorquages
        attr_rem = getattr(self, f"txt_remorq{i}", None)
        if attr_rem:
            new_tarif_data[f'remorq{i}'] = float(attr_rem.text() or 0)

    # 3. Envoyer le dictionnaire complet (les 62 champs) au contrôleur
    success, message = self.controller.tarifs.create_tarif(new_tarif_data)
