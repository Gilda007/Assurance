from PySide6.QtWidgets import (QWidget, QVBoxLayout, QHBoxLayout, QTableWidget, 
                             ...
                             QLineEdit, QComboBox, QLabel, QFileDialog, QProgressDialog, QApplication)
...
class TarifMainView(QWidget):
    ...
    def on_import_click(self):
        file_path, _ = QFileDialog.getOpenFileName(
            self, "Sélectionner le fichier", "", "Fichiers de données (*.csv *.xlsx *.xls)"
        )
        ...
        progress = QProgressDialog("Préparation de l'importation...", "Annuler", 0, 100, self)
        progress.setWindowModality(Qt.WindowModal)
        progress.setWindowTitle("Importation massive")
        progress.setMinimumDuration(0) # Apparaît immédiatement
        progress.show()

        # 2. Fonction de rappel (callback) pour mettre à jour la barre
        def update_progress(current, total):
            percent = int((current / total) * 100)
            progress.setValue(percent)
            progress.setLabelText(f"Importation : {current} / {total} tarifs...")
            # Forcer l'application à traiter les événements pour éviter le gel
            QApplication.processEvents()
            
            if progress.wasCanceled():
                return # Optionnel : gérer l'arrêt

        # 3. Lancer l'import
        u_id = self.user.id if hasattr(self.user, 'id') else 1
        success, message = self.controller.tarifs.import_tarifs_from_file(
            file_path, u_id, progress_callback=update_progress
        )
