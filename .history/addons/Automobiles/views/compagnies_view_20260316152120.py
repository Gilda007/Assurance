...
class CompanyTariffView(QWidget):
    ...
    def on_add_click(self, form_data, user_info):
        dialog = FormulaireCreationCompagnie(self, controller=self.controller, current_user=self.user)
        user_login = user_info.login if hasattr(user_info, 'login') else str(user_info)
        
        if dialog.exec_():
            data = dialog.get_data()
            # Appel au bon contrôleur (compagnies)
            success, message = self.controller.compagnies.create_compagnie(data, self.user.login)            
            if success:
                # CORRECTION ICI : Appelez la méthode de rafraîchissement des COMPAGNIES
                # Ne pas appeler display_contacts qui n'existe pas ici
                self.load_compagnies_data() 
                AlertManager.show_success(self, "Succès", "Compagnie ajoutée")
            else:
