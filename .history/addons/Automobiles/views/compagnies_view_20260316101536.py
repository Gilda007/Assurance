...
from addons.Automobiles.views.tarif_form_view import FormulaireCreationTarif
...
class CompanyTariffView(QWidget):
    ...
    def setup_ui(self):
        ...
        self.btn_save_tariffs.clicked.connect(self.export_to_pdf)
    ...
    def on_add_tarif_click(self):
        dialog = FormulaireCreationTarif(self, self.controller, self.current_user)
        # Très important pour que exec_() renvoie True quand on clique sur valider
        dialog.btn_save.clicked.connect(dialog.accept) 
        
        if dialog.exec_():
            data = dialog.get_data()
            # Ici votre appel au controller pour sauvegarder les tarifs
            print(f"Enregistrement du barème : {data['nom_bareme']}")
