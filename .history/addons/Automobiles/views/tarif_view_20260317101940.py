...
class TarifMainView(QWidget):
    ...
    def on_import_click(self):
        file_path, _ = QFileDialog.getOpenFileName(self, "Importer Tarifs", "", "Excel/CSV (*.csv *.xlsx *.xls)")
        if not file_path: return

        # Création de la barre
        progress = QProgressDialog("Initialisation de l'import...", "Annuler", 0, 100, self)
        ...
        def update_ui(current, total):
            ...
            progress.setLabelText(f"Traitement : {current} / {total} tarifs...")
            QApplication.processEvents() # Empêche le gel de l'UI

        # Exécution
        user_id = self.user.id if hasattr(self.user, 'id') else 1
        success, message = self.controller.tarifs.import_tarifs_from_file(
            file_path, user_id, progress_callback=update_ui
        )

        progress.setValue(100)
        if success:
            AlertManager.show_success(self, "Import terminé", message)
            self.refresh_table()
        else:
