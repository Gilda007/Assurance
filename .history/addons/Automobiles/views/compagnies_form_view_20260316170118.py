...
class FormulaireCreationCompagnie(QDialog):
    ...
    def set_read_only(self, is_read_only=True):
        """Désactive l'édition pour le mode visualisation."""
        # Liste de tous nos champs d'entrée
        fields = [
            self.input_code, self.input_nom, self.input_email, 
            self.input_telephone, self.input_adresse, 
            self.input_num_debut, self.input_num_fin
        ]
        
        for field in fields:
            field.setReadOnly(is_read_only)
            if is_read_only:
                field.setStyleSheet(field.styleSheet() + "background-color: #f8fafc; border: none;")

        # Cacher le bouton enregistrer en mode lecture seule
        if is_read_only:
            self.btn_save.hide()
