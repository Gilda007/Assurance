...
class CompanyTariffView(QWidget):
    ...
    # --- CORRECTION DES SIGNATURES ---

    def on_edit_click(self, cie_id):
        """Prend maintenant l'ID en argument."""
        print(f"Modification de la compagnie ID: {cie_id}")
        # Logique pour ouvrir le formulaire en mode édition...
    ...
    def on_print_click(self, cie_id):
        """Prend maintenant l'ID en argument."""
        print(f"Impression de la compagnie ID: {cie_id}")

    def on_delete_click(self, cie_id):
        """Prend maintenant l'ID en argument."""
        confirm = AlertManager.ask_confirmation(self, "Suppression", "Désactiver cette compagnie ?")
        if confirm:
            user_id = self.user.id if hasattr(self.user, 'id') else 1
            success, msg = self.controller.compagnies.delete_compagnie_logic(cie_id, user_id)
            if success:
