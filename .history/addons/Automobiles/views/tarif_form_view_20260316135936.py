...
class CompanyTarifForm(QDialog):
    def __init__(self, parent=None, controller=None, current_user=None):
        ...
        self.setWindowFlags(Qt.FramelessWindowHint | Qt.Dialog)
        self.setAttribute(Qt.WA_TranslucentBackground)
        self.setMinimumWidth(1000) # Un peu plus large pour la grille triple
        self.setFixedHeight(850)
    ...
    def setup_ui(self):
        ...
        h_lay = QHBoxLayout(header)
        h_lay.setContentsMargins(35, 0, 35, 0)
        title = QLabel("Saisie du Barème Tarifaire")
        ...
        grid_id.addWidget(self._reg_input("Nom_Cie", "Compagnie", "SANLAM"), 0, 1)
        grid_id.addWidget(self._reg_input("Lib_Tarif", "Libellé", "AMBULANCES..."), 0, 2)
        grid_id.addWidget(self._reg_input("Zone", "Zone", "A"), 1, 0)
        grid_id.addWidget(self._reg_input("Categorie", "Catégorie", "0800"), 1, 1)
        grid_id.addWidget(self._reg_input("Nbre_Place", "Places", "5"), 1, 2)
        s_lay.addLayout(grid_id)

        # --- GRILLE COMMUNE (SECTION 2, 3, 4) ---
        s_lay.addWidget(self._lbl_sec("GRILLE TARIFAIRE DÉTAILLÉE (RC / REMORQUAGE / INFLAMMABLE)"))
        
        # Grid layout pour simuler le tableau de la capture
        grid_main = QGridLayout()
        grid_main.setSpacing(10)

        # En-têtes de colonnes
        headers = ["N°", "PRIME RC", "REMORQUAGE", "INFLAMMABLE"]
        for col, text in enumerate(headers):
            h_lbl = QLabel(text)
            h_lbl.setStyleSheet("color: #64748b; font-size: 10px; font-weight: 800; padding-bottom: 5px;")
            h_lbl.setAlignment(Qt.AlignCenter if col > 0 else Qt.AlignLeft)
            grid_main.addWidget(h_lbl, 0, col)

        # Génération des 10 lignes
        for row_idx in range(1, 11):
            # Label de ligne
            row_lbl = QLabel(f"Ligne {row_idx}")
            row_lbl.setStyleSheet("color: #1e293b; font-weight: 700; font-size: 11px;")
            grid_main.addWidget(row_lbl, row_idx, 0)

            # Champs de saisie (clés basées sur votre CSV)
            grid_main.addWidget(self._create_grid_cell(f"Prime{row_idx}"), row_idx, 1)
            grid_main.addWidget(self._create_grid_cell(f"Remorq{row_idx}"), row_idx, 2)
            grid_main.addWidget(self._create_grid_cell(f"Inflamble{row_idx}"), row_idx, 3)

        s_lay.addLayout(grid_main)

        # 5. ÉNERGIE (ESSENCE / DIESEL)
        s_lay.addWidget(self._lbl_sec("PUISSANCE & ÉNERGIE"))
        grid_eng = QGridLayout()
        for i in range(1, 6):
            grid_eng.addWidget(self._reg_input(f"Essence{i}", f"Essence {i}", "0.0"), i-1, 0)
            grid_eng.addWidget(self._reg_input(f"Diesel{i}", f"Diesel {i}", "0.0"), i-1, 1)
        s_lay.addLayout(grid_eng)
    ...
    def _create_grid_cell(self, key):
        """Crée un champ QLineEdit compact pour la grille"""
        edit = QLineEdit()
    ...
    def _reg_input(self, key, label, placeholder):
        ...
        edit.setStyleSheet("QLineEdit { background: #f8fafc; border: none; border-bottom: 2px solid #f1f5f9; border-radius: 6px; padding-left: 10px; } QLineEdit:focus { border-bottom: 2px solid #3b82f6; background: white; }")
