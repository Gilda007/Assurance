...
class CompanyTarifForm(QDialog):
    ...
    def setup_ui(self):
        ...
        grid_id = QGridLayout()
        grid_id.addWidget(self._create_input("Cie", "Code Compagnie", "1111"), 0, 0)
        grid_id.addWidget(self._create_input("Nom_Cie", "Nom Compagnie", "SANLAM"), 0, 1)
        grid_id.addWidget(self._create_input("Lib_Tarif", "Libellé du Tarif", "AMBULANCES..."), 1, 0)
        grid_id.addWidget(self._create_input("Categorie", "Catégorie", "0800"), 1, 1)
        grid_id.addWidget(self._create_input("Zone", "Zone Géographique", "A"), 2, 0)
        grid_id.addWidget(self._create_input("Nbre_Place", "Nombre de Places", "5"), 2, 1)
        ...
        for i in range(1, 11):
            grid_primes.addWidget(self._create_input(f"Prime{i}", f"Prime {i}", "0.0"), (i-1)//5, (i-1)%5)
        ...
        for i in range(1, 11):
            grid_rem.addWidget(self._create_input(f"Remorq{i}", f"Remorquage {i}", "0.0"), (i-1)//5, (i-1)%5)
        ...
        for i in range(1, 11):
            grid_inf.addWidget(self._create_input(f"Inflamble{i}", f"Inflammable {i}", "0.0"), (i-1)//5, (i-1)%5)
        ...
        for i in range(1, 11):
            grid_ess.addWidget(self._create_input(f"Essence{i}", f"Essence {i}", "0.0"), (i-1)//5, (i-1)%5)
        ...
        for i in range(1, 11):
            grid_die.addWidget(self._create_input(f"Diesel{i}", f"Diesel {i}", "0.0"), (i-1)//5, (i-1)%5)
        ...
        grid_limit.addWidget(self._create_input("Max_Corpo", "Maximum Corporel", "Illimité"), 0, 0)
        grid_limit.addWidget(self._create_input("Max_Materiel", "Maximum Matériel", "0.0"), 0, 1)
        grid_limit.addWidget(self._create_input("Surprime1", "Surprime Optionnelle 1", "0.0"), 1, 0)
        grid_limit.addWidget(self._create_input("Surprime2", "Surprime Optionnelle 2", "0.0"), 1, 1)
