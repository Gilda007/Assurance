...
class CompanyTariffView(QWidget):
   ...
   def on_add_click(self):
        """Déclenche l'ouverture du formulaire et gère la sauvegarde."""
        # 1. Instanciation du formulaire
        dialog = FormulaireCreationCompagnie(self, controller=self.controller, current_user=self.user)
        
        # 2. Exécution du dialogue (bloquant jusqu'à 'Enregistrer' ou 'Annuler')
        if dialog.exec_():
            # 3. Récupération des données saisies par l'utilisateur
            data = dialog.get_data()
            
            # 4. Extraction sécurisée du login pour l'audit (évite l'erreur "can't adapt type User")
            # On vérifie si self.user est un objet ou déjà une chaîne
            user_login = self.user.login if hasattr(self.user, 'login') else str(self.user)
            
            # 5. Appel au contrôleur pour l'insertion en base de données
            success, message = self.controller.compagnies.create_compagnie(data, user_login)            
            ...
            if success:
                ...
                AlertManager.show_success(self, "Succès", "La compagnie a été ajoutée avec succès.")
            else:
                # 7. Gestion des erreurs (doublons, connexion, etc.)
