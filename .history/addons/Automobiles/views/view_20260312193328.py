...
class VehicleMainView(QWidget):
    ...
    def load_data(self):
        ...
        for i, v in enumerate(vehicles):
            self.table.setItem(i, 0, QTableWidgetItem(v.get('immatriculation', '---')))
            self.table.setItem(i, 1, QTableWidgetItem(f"{v.get('marque')} {v.get('modele')}"))
            self.table.setItem(i, 2, QTableWidgetItem(v.get('client_name', 'Client inconnu')))
            
            # Badge de statut
            statut = v.get('status', 'Actif')
