...
class CompanyTariffView(QWidget):
    ...
    def load_compagnies_data(self):
        ...
        for cie in compagnies:
            ...
            actions_layout.setStyleSheet(""" background: None, border: None""")
    ...
    def filter_compagnies(self, text):
        """Filtre les widgets de la sidebar selon le texte saisi."""
        text = text.lower()
        for i in range(self.comp_vbox.count()):
            item = self.comp_vbox.itemAt(i).widget()
            if item:
                # On récupère le nom et le code dans le widget pour comparer
                # (Ou on stocke les infos dans item.property('data'))
                nom = item.findChild(QLabel).text().lower()
                item.setVisible(text in nom)
