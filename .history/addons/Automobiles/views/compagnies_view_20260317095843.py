...
class CompanyTariffView(QWidget):
    ...
    def setup_ui(self):
        ...
        side_title.setStyleSheet("font-size: 18px; font-weight: 800; color: #0f172a; margin-bottom: 10px; border: None")
        ...
        self.selected_label = QLabel("AMS Assurance - Gestion des Tarifs")
        self.selected_label.setStyleSheet("font-size: 24px; font-weight: 800; color: #0f172a; background-color: None")
        ...
        self.btn_import = QPushButton("📥 Import les tarif")
        ...
        """)
        self.btn_import.clicked.connect(self.on_import_excel_click)
        
        top_header.addWidget(self.selected_label)
        top_header.addStretch()
        top_header.addWidget(self.btn_save_tariffs)

        top_header.addWidget(self.btn_import)
    ...
    def _create_company_item(self, cie):
        """Crée un widget de type 'Card' pour la sidebar avec gestion d'état."""
        container = QFrame()
        container.setObjectName("CompanyItem")
        # On stocke l'ID dans une propriété pour le retrouver facilement plus tard
        container.setProperty("cie_id", cie.id)
        container.setCursor(Qt.PointingHandCursor)
        
        # Style moderne avec gestion de l'état 'active' (sélectionné)
        container.setStyleSheet("""
            ...
            QFrame#CompanyItem[active="true"] {
        ...
        # --- Ligne 1 : Titre (Nom) et Badge (Code) ---
        top_layout = QHBoxLayout()
        top_layout.setContentsMargins(0, 0, 0, 0)
        
        # Nom de la compagnie (Tronqué si trop long pour garder l'UI propre)
        nom_text = cie.nom.upper()
        if len(nom_text) > 20: nom_text = nom_text[:18] + ".."
        
        nom_label = QLabel(nom_text)
        nom_label.setStyleSheet("font-weight: 700; color: #1e293b; font-size: 12px; border: none; background: transparent;")
        
        code_badge = QLabel(f" {cie.code} ")
        code_badge.setAlignment(Qt.AlignCenter)
        ...
        # --- Ligne 2 : Info Flotte avec icône discrète ---
        flotte_info = f"📑 Flotte : {cie.num_debut or '—'} à {cie.num_fin or '—'}"
        flotte_label = QLabel(flotte_info)
        flotte_label.setStyleSheet("color: #64748b; font-size: 10px; border: none; background: transparent;")
        layout.addWidget(flotte_label)

        # --- Événement de clic ---
        # On utilise une fonction intermédiaire pour gérer le style de sélection
        container.mousePressEvent = lambda event, c_id=cie.id, widget=container: self._on_sidebar_item_clicked(c_id, widget)
        
        return container 
      
    def filter_compagnies(self, text):
        """Filtre les compagnies affichées dans la sidebar."""
        search_text = text.lower().strip()
        
        # Parcourir tous les items dans le layout de la liste
        for i in range(self.comp_vbox.count()):
            item = self.comp_vbox.itemAt(i)
            widget = item.widget()
            
            if widget:
                # Récupérer les données stockées ou extraire le texte des labels
                # Option A : Si vous avez utilisé setProperty("cie_nom", ...)
                # Option B : Chercher les labels à l'intérieur du widget
                labels = widget.findChildren(QLabel)
                
                # On concatène le texte de tous les labels (Nom + Code) pour la recherche
                content = " ".join([l.text().lower() for l in labels])
    ...
    def _on_sidebar_item_clicked(self, cie_id, widget):
        """Gère le clic sur un item de la sidebar : surbrillance et affichage."""
        # 1. Retirer le style "active" de tous les autres widgets
        for i in range(self.comp_vbox.count()):
            w = self.comp_vbox.itemAt(i).widget()
            if w:
                w.setProperty("active", False)
                w.style().unpolish(w)
                w.style().polish(w)

        # 2. Appliquer le style "active" au widget cliqué
        widget.setProperty("active", True)
        widget.style().unpolish(widget)
        widget.style().polish(widget)

        # 3. Ouvrir la vue (Visualisation ou sélection pour les tarifs)
        self.on_view_click(cie_id)

    def on_import_excel_click(self):
        ...
        if file_path:
            u_id = self.user.id if hasattr(self.user, 'id') else 1
            success, message = self.controller.compagnies.import_from_excel(file_path, u_id)
