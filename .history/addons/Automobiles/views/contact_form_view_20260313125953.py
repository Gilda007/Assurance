...
#                              QScrollArea, QDateEdit, QWidget, QMessageBox)
...
#         # --- DROITE : MODULE CAMERA ---
#         photo_panel = QVBoxLayout()
#         photo_panel.setSpacing(10)
        
#         self.lbl_camera = QLabel("Flux Caméra")
#         self.lbl_camera.setObjectName("PhotoFrame")
#         self.lbl_camera.setFixedSize(320, 240)
#         self.lbl_camera.setAlignment(Qt.AlignCenter)

#         self.btn_start_cam = QPushButton("📷 Démarrer Caméra")
#         self.btn_capture = QPushButton("🎯 Prendre la Photo")
#         self.btn_capture.setEnabled(False)
        
#         # Styles boutons photo
#         self.btn_start_cam.setStyleSheet("background: #1e293b; color: white; padding: 8px; border-radius: 5px;")
#         self.btn_capture.setStyleSheet("background: #10b981; color: white; padding: 8px; border-radius: 5px;")

#         photo_panel.addWidget(QLabel("📸 IDENTIFICATION PHOTO"))
#         photo_panel.addWidget(self.lbl_camera)
#         photo_panel.addWidget(self.btn_start_cam)
#         photo_panel.addWidget(self.btn_capture)
#         photo_panel.addStretch()

#         body_layout.addLayout(photo_panel, 1) # Prend 1/3 de l'espace
#         # --- CONNEXIONS PHOTO ---
...
#         self._add_field("Adresse Résidentielle", self.adresse, 7, 0)
...
#         """
#         Ajoute un label et son widget associé dans la grille.
#         Le paramètre colspan permet au champ de s'étaler sur plusieurs colonnes.
#         """
#         vbox = QVBoxLayout()
#         vbox.setSpacing(4) # Espace serré entre le label et son champ
        
#         lbl = QLabel(label_text)
#         # On peut ajouter un style spécifique aux labels ici
#         lbl.setStyleSheet("color: #475569; font-weight: 600; font-size: 12px;")
        
#         vbox.addWidget(lbl)
#         vbox.addWidget(widget)
        
#         # Signature de addLayout dans un QGridLayout :
...
#             self.cap = cv2.VideoCapture(0)
#             self.timer.start(30)
#             self.btn_start_cam.setText("🛑 Arrêter Caméra")
...
#             self.cap.release()
#         self.cap = None
#         self.btn_start_cam.setText("📷 Démarrer Caméra")
#         self.btn_capture.setEnabled(False)
...
#             self.lbl_camera.setPixmap(QPixmap.fromImage(img).scaled(320, 240, Qt.KeepAspectRatio))

#     def take_photo(self):
#         ret, frame = self.cap.read()
#         if ret:
#             self.captured_image = frame
#             # On affiche un flash ou on change le bord pour confirmer
#             self.lbl_camera.setStyleSheet("border: 4px solid #10b981; border-radius: 10px;")
#             self.stop_camera()
#             QMessageBox.information(self, "Photo", "Capture réussie !")

#     # --- Helpers de structure (les mêmes qu'avant) ---
#     def _add_section_title(self, text, row):
#         lbl = QLabel(text); lbl.setProperty("class", "SectionTitle")
#         self.grid.addWidget(lbl, row, 0, 1, 3)

#     def _add_field(self, label, widget, row, col):
#         v = QVBoxLayout(); v.addWidget(QLabel(label)); v.addWidget(widget)
...
class ContactForm(QDialog):
    ...
    def setup_ui(self):
        ...
        main_title = QLabel("Fiche Client Digitale")
        main_title.setStyleSheet("font-size: 24px; color: #1e293b; font-weight: 900; border: none;")
        subtitle = QLabel("Enregistrement des données et capture biométrique")
        subtitle.setStyleSheet("color: #94a3b8; font-size: 13px; font-weight: normal; border: none;")
        ...
        self.btn_close.setStyleSheet("background: #fee2e2; color: #ef4444; border-radius: 20px; font-weight: bold; border: none;")
        ...
        self.grid = QGridLayout(form_content)
        self.grid.setSpacing(20)
        self.grid.setContentsMargins(0, 0, 20, 0)

        # Section 1 : Identité
        self._add_section_title("📋 Identification de base", 0)
        self.type_client = QComboBox(); self.type_client.addItems(["Standard", "VIP"])
        self.type_contact = QComboBox(); self.type_contact.addItems(["Physique", "Morale"])
        self.civilite = QComboBox(); self.civilite.addItems(["M.", "Mme", "Mlle"])
        
        self._add_field("Profil Client", self.type_client, 1, 0)
        self._add_field("Type Entité", self.type_contact, 1, 1)
        self._add_field("Civilité", self.civilite, 1, 2)

        self.nom = QLineEdit(); self.nom.setPlaceholderText("Nom ou Raison Sociale")
        self.prenom = QLineEdit(); self.prenom.setPlaceholderText("Prénom")
        self._add_field("Nom / Dénomination", self.nom, 2, 0, colspan=2)
        self._add_field("Prénom", self.prenom, 2, 2)

        # Section 2 : Fiscalité & Perso
        self._add_section_title("🔑 Documents & Fiscalité", 3)
        self.num_contribuable = QLineEdit(); self.num_contribuable.setPlaceholderText("N° IFU / Contribuable")
        self.date_naissance = QDateEdit(); self.date_naissance.setCalendarPopup(True)
        self.nationalite = QLineEdit(); self.nationalite.setPlaceholderText("Ex: Sénégalaise")
        
        self._add_field("N° Contribuable", self.num_contribuable, 4, 0)
        self._add_field("Date Naissance / Création", self.date_naissance, 4, 1)
        self._add_field("Nationalité", self.nationalite, 4, 2)

        # Section 3 : Coordonnées
        self._add_section_title("📞 Coordonnées de contact", 5)
        self.telephone = QLineEdit(); self.fax = QLineEdit(); self.email = QLineEdit()
        self.adresse = QLineEdit(); self.adresse.setPlaceholderText("Adresse complète du domicile ou siège")
        
        self._add_field("Téléphone", self.telephone, 6, 0)
        self._add_field("Fax / Fixe", self.fax, 6, 1)
        self._add_field("E-mail Professionnel", self.email, 6, 2)
        ...
        # DROITE : CAMERA PANEL
        cam_panel = QFrame()
        cam_panel.setStyleSheet("background-color: #f8fafc; border-radius: 20px; border: 1px solid #f1f5f9;")
        cam_layout = QVBoxLayout(cam_panel)
        cam_layout.setContentsMargins(20, 20, 20, 20)

        self.lbl_camera = QLabel("Flux Caméra Inactif")
        self.lbl_camera.setFixedSize(320, 240)
        self.lbl_camera.setAlignment(Qt.AlignCenter)
        ...
        cam_layout.addWidget(QLabel("CAPTURE BIOMÉTRIQUE"), alignment=Qt.AlignCenter)
        cam_layout.addWidget(self.lbl_camera)
        
        self.btn_start_cam = QPushButton("Démarrer Caméra")
        self.btn_start_cam.setObjectName("ActionBtn")
        self.btn_start_cam.setStyleSheet("background: #1e293b; color: white;")
        
        self.btn_capture = QPushButton("Capturer l'image")
        self.btn_capture.setObjectName("ActionBtn")
        self.btn_capture.setStyleSheet("background: #10b981; color: white;")
        self.btn_capture.setEnabled(False)
    ...
    def toggle_camera(self):
        if self.cap is None:
            ...
            self.timer.start(30)
            self.btn_start_cam.setText("Arrêter Caméra")
            self.btn_start_cam.setStyleSheet("background: #f1f5f9; color: #475569;")
            self.btn_capture.setEnabled(True)
        else:
            self.stop_camera()
    ...
    def update_frame(self):
        ...
        if ret:
            ...
            h, w, ch = frame.shape
            img = QImage(frame.data, w, h, ch * w, QImage.Format_RGB888)
            self.lbl_camera.setPixmap(QPixmap.fromImage(img).scaled(320, 240, Qt.KeepAspectRatioByExpanding, Qt.SmoothTransformation))

    def take_photo(self):
        ret, frame = self.cap.read()
        if ret:
            self.captured_image = frame
            self.lbl_camera.setStyleSheet("border: 4px solid #10b981; border-radius: 15px;")
            self.stop_camera()
            QMessageBox.information(self, "Succès", "Photo capturée avec succès.")

    def save_data(self):
        # Ici votre logique de sauvegarde via le contrôleur
        if not self.nom.text():
            QMessageBox.warning(self, "Erreur", "Veuillez saisir au moins un nom.")
            return
        self.accept()