...
class CompanyTariffView(QWidget):
    ...
    def _create_action_btn(self, icon_text, color, tooltip):
        ...
        btn.setStyleSheet(f"""
            QPushButton {{ 
                background-color: transparent; border-radius: 5px; font-size: 14px;
    ...
    def on_delete_click(self, cie_id):
        ...
        res = AlertManager.ask_confirmation(self, "Suppression", "Voulez-vous désactiver cette compagnie ?")
        if res:
            user_id = self.user.id if hasattr(self.user, 'id') else 1
            success, msg = self.controller.compagnies.delete_compagnie_logic(cie_id, user_id)
            if success:
...
3.
    ...
    def on_edit_click(self):
        """Action déclenchée lors du clic sur 'Modifier'"""
        # 1. Récupérer l'ID de la ligne sélectionnée (ex: stocké dans une colonne cachée ou data)
        selected_row = self.table.currentRow()
        if selected_row < 0:
            AlertManager.show_warning(self, "Sélection", "Veuillez choisir une compagnie à modifier.")
            return

        # On suppose que l'ID est stocké dans la colonne 0 (cachée ou non)
        compagnie_id = int(self.table.item(selected_row, 0).text())
        
        # 2. Récupérer les données actuelles (soit du tableau, soit de la DB via contrôleur)
        # Idéalement, on demande au contrôleur les infos fraîches
        compagnie_data = self.controller.compagnies.get_compagnie_by_id(compagnie_id)

        # 3. Ouvrir le formulaire en mode édition
        dialog = FormulaireCreationCompagnie(
            self, 
            controller=self.controller, 
            current_user=self.user,
            data=compagnie_data  # On passe les données pour pré-remplir
        )
        
        if dialog.exec_():
            new_data = dialog.get_data()
            
            # Extraction de l'ID utilisateur (Integer) comme pour le Create
            user_id = self.user.id if hasattr(self.user, 'id') else int(self.user)
            
            # 4. Appel de la méthode update
            success, message = self.controller.compagnies.update_compagnie(
                compagnie_id, 
                new_data, 
                user_id
            )
            ...
            if success:
                ...
                AlertManager.show_success(self, "Succès", "Modification enregistrée.")
            else:
